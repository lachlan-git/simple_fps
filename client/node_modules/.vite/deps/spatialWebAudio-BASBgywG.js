import { n as Quaternion, o as Vector3, t as Matrix } from "./math.vector.pure-BrntvQ1d.js";
import { t as PrecisionDate } from "./precisionDate-B6nrhdf_.js";
import { d as _WebAudioParameterComponent, o as _GetVolumeAudioSubNode, s as _AbstractAudioSubNode, t as _WebAudioBaseSubGraph } from "./webAudioBaseSubGraph-C4MLehLJ.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/components/spatialAudioAttacherComponent.js
/**
...
var _SpatialAudioAttacherComponent = class {
	...
	*/
	attach(sceneNode, useBoundingBox, attachmentType) {
		if (this._sceneNode === sceneNode) return;
		this.detach();
		if (!sceneNode) return;
	...
	*/
	update() {
		...
		if (updatesPosition) {
			if (this._useBoundingBox && this._sceneNode.getBoundingInfo) this._position.copyFrom(this._sceneNode.getBoundingInfo().boundingBox.centerWorld);
			else this._sceneNode?.getWorldMatrix().getTranslationToRef(this._position);
		...
		if (!updatesPosition && "panningEnabled" in this._spatialAudioNode && !this._spatialAudioNode.panningEnabled) this._spatialAudioNode._updatePosition();
	}
};
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/webAudio/components/spatialWebAudioUpdaterComponent.js
/** @internal */
		...
		*/
		...
		const update = () => {
			...
			if (0 < this.minUpdateTime) {
				...
				if (this._lastUpdateTime && now - this._lastUpdateTime < this.minUpdateTime * 1e3) skipUpdate = true;
				this._lastUpdateTime = now;
			}
			if (!skipUpdate) parent.update();
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/subProperties/abstractSpatialAudio.js
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/subProperties/abstractStereoAudio.js
var _StereoAudioDefaults = { pan: 0 };
/**
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/subNodes/stereoAudioSubNode.js
/** @internal */
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/subProperties/stereoAudio.js
/** @internal */
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/subNodes/spatialAudioSubNode.js
/** @internal */
var _SpatialAudioSubNode = class extends _AbstractAudioSubNode {
	...
	attach(sceneNode, useBoundingBox, attachmentType) {
		this.detach();
		if (!this._attacherComponent) this._attacherComponent = new _SpatialAudioAttacherComponent(this);
	...
	setOptions(options) {
		...
		if (options.spatialPosition) this.position = options.spatialPosition.clone();
		if (options.spatialRotationQuaternion) this.rotationQuaternion = options.spatialRotationQuaternion.clone();
		else if (options.spatialRotation) this.rotation = options.spatialRotation.clone();
		else this.rotationQuaternion = _SpatialAudioDefaults.rotationQuaternion.clone();
		this.update();
	}
	/** @internal */
	update() {
		if (this.isAttached) this._attacherComponent?.update();
		else {
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/webAudio/subNodes/spatialWebAudioSubNode.js
...
var _SpatialWebAudioSubNode = class extends _SpatialAudioSubNode {
	...
	set distanceModel(value) {
		...
		this.node.maxDistance = maxDistance + .001;
	...
	set panningEnabled(value) {
		if (this._panningEnabled === value) return;
	...
	_connectActiveInput() {
		this._inputNode.disconnect();
		if (this._panningEnabled) this._inputNode.connect(this.node);
		else this._inputNode.connect(this._attenuationNode);
	}
	_updateAttenuation() {
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/webAudio/subNodes/stereoWebAudioSubNode.js
/** @internal */
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/webAudio/subNodes/webAudioBusAndSoundSubGraph.js
/** @internal */
var _WebAudioBusAndSoundSubGraph = class extends _WebAudioBaseSubGraph {
	...
	async initAsync(options) {
		...
		if (hasSpatialOptions = _HasSpatialAudioOptions(options)) await this.createAndAddSubNodeAsync("Spatial");
		if (hasStereoOptions = _HasStereoAudioOptions(options)) await this.createAndAddSubNodeAsync("Stereo");
		await this._createSubNodePromisesResolvedAsync();
		if (hasSpatialOptions) _GetSpatialAudioSubNode(this)?.setOptions(options);
		if (hasStereoOptions) _GetStereoAudioSubNode(this)?.setOptions(options);
	}
	/** @internal */
	get _inNode() {
		return this._inputNode;
	...
	_createSubNode(name) {
		try {
			return super._createSubNode(name);
		} catch (e) {}
		switch (name) {
			case "Spatial": return _CreateSpatialAudioSubNodeAsync(this._owner.engine);
			case "Stereo": return _CreateStereoAudioSubNodeAsync(this._owner.engine);
	...
	_onSubNodesChanged() {
		...
		if (spatialNode && spatialNode.getClassName() !== "_SpatialWebAudioSubNode") throw new Error("Not a WebAudio subnode.");
		if (stereoNode && stereoNode.getClassName() !== "_StereoWebAudioSubNode") throw new Error("Not a WebAudio subnode.");
		if (volumeNode && volumeNode.getClassName() !== "_VolumeWebAudioSubNode") throw new Error("Not a WebAudio subnode.");
		if (spatialNode) {
			spatialNode.disconnectAll();
			if (volumeNode) spatialNode.connect(volumeNode);
		}
		if (stereoNode) {
			stereoNode.disconnectAll();
			if (volumeNode) stereoNode.connect(volumeNode);
		}
		...
		let inSubNode = null;
		let inNode;
		if (this._rootNode) inNode = this._rootNode;
		else {
			if (spatialNode) inSubNode = spatialNode;
			else if (stereoNode) inSubNode = stereoNode;
			else if (volumeNode) inSubNode = volumeNode;
		...
		if (this._inputNode !== inNode) {
			if (this._inputNode && this._upstreamNodes) {
				...
				for (let next = it.next(); !next.done; next = it.next()) next.value._outNode?.disconnect(this._inputNode);
			}
			this._inputNode = inNode;
			if (inNode && this._upstreamNodes) {
				const it = this._upstreamNodes.values();
				for (let next = it.next(); !next.done; next = it.next()) next.value._outNode?.connect(inNode);
			}
		}
	}
};
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/abstractAudio/subProperties/spatialAudio.js
/** @internal */
var _SpatialAudio = class extends AbstractSpatialAudio {
	...
	constructor(subGraph) {
		...
		if (subNode) {
			...
			subGraph.createAndAddSubNodeAsync("Spatial");
		}
		this._subGraph = subGraph;
	}
	/** @internal */
	...
	/** @internal */
	get attachedNode() {
		return this._subGraph.getSubNode("Spatial")?.attachedNode ?? null;
	}
	/** @internal */
	get useBoundingBox() {
		return this._subGraph.getSubNode("Spatial")?.useBoundingBox ?? false;
	}
	/** @internal */
	get attachmentType() {
		return this._subGraph.getSubNode("Spatial")?.attachmentType ?? 3;
	}
	/** @internal */
	get maxDistance() {
	...
	*/
	...
	update() {
		...
		if (!subNode) return;
		if (subNode.isAttached) subNode.update();
		else {
	...
	_updatePosition(subNode = null) {
		...
		if (!subNode.position.equalsWithEpsilon(this._position)) {
			subNode.position.copyFrom(this._position);
			subNode._updatePosition();
		} else if (!subNode.panningEnabled) subNode._updatePosition();
	}
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/AudioV2/webAudio/subProperties/spatialWebAudio.js
/** @internal */
...
//#endregion
export { _HasStereoAudioOptions as a, _HasSpatialAudioOptions as c, _SpatialAudioAttacherComponent as d, AbstractStereoAudio as i, _SpatialAudioDefaults as l, _WebAudioBusAndSoundSubGraph as n, _StereoAudioDefaults as o, _StereoAudio as r, AbstractSpatialAudio as s, _SpatialWebAudio as t, _SpatialWebAudioUpdaterComponent as u };
