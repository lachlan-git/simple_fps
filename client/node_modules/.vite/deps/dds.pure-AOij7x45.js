import { E as __exportAll, t as Clamp } from "./math.scalar.functions-DvvDSyEy.js";
import { t as Logger } from "./logger-CVZpwFO0.js";
import { r as ToHalfFloat, t as FromHalfFloat } from "./halfFloat-CuNBUgay.js";
import { t as CubeMapToSphericalPolynomialTools } from "./cubemapToSphericalPolynomial-iH8V2kcC.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Misc/dds.pure.js
/** This file must only contain pure code and pure imports */
var dds_pure_exports = /* @__PURE__ */ __exportAll({ DDSTools: () => DDSTools });
...
var FOURCC_DXT1 = /*#__PURE__*/ FourCCToInt32("DXT1");
var FOURCC_DXT3 = /*#__PURE__*/ FourCCToInt32("DXT3");
var FOURCC_DXT5 = /*#__PURE__*/ FourCCToInt32("DXT5");
var FOURCC_DX10 = /*#__PURE__*/ FourCCToInt32("DX10");
...
var DDSTools = class DDSTools {
	...
	*/
	static GetDDSInfo(data) {
		...
		const extendedHeader = new Int32Array(data.buffer, data.byteOffset, 35);
		let mipmapCount = 1;
		if (header[off_flags] & DDSD_MIPMAPCOUNT) mipmapCount = Math.max(1, header[off_mipmapCount]);
	...
	static _GetHalfFloatAsFloatRGBAArrayBuffer(width, height, dataOffset, dataLength, arrayBuffer, lod) {
		...
		for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
			...
			if (DDSTools.StoreLODInAlphaChannel) destArray[index + 3] = lod;
			else destArray[index + 3] = FromHalfFloat(srcData[srcPos + 3]);
			index += 4;
		}
		return destArray;
	}
	...
	static _GetFloatAsHalfFloatRGBAArrayBuffer(width, height, dataOffset, dataLength, arrayBuffer, lod) {
		...
		for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
			...
			if (DDSTools.StoreLODInAlphaChannel) destArray[index + 3] = ToHalfFloat(lod);
			else destArray[index + 3] = ToHalfFloat(srcData[index + 3]);
			index += 4;
		}
		return destArray;
	}
	static _GetFloatAsUIntRGBAArrayBuffer(width, height, dataOffset, dataLength, arrayBuffer, lod) {
		...
		for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
			...
			if (DDSTools.StoreLODInAlphaChannel) destArray[index + 3] = lod;
			else destArray[index + 3] = Clamp(srcData[srcPos + 3]) * 255;
			index += 4;
		}
		return destArray;
	}
	static _GetHalfFloatAsUIntRGBAArrayBuffer(width, height, dataOffset, dataLength, arrayBuffer, lod) {
		...
		for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
			...
			if (DDSTools.StoreLODInAlphaChannel) destArray[index + 3] = lod;
			else destArray[index + 3] = Clamp(FromHalfFloat(srcData[srcPos + 3])) * 255;
			index += 4;
		}
		return destArray;
	}
	...
	static _ExtractLongWordOrder(value) {
		if (value === 0 || value === 255 || value === -16777216) return 0;
	...
	*/
	static UploadDDSLevels(engine, texture, data, info, loadMipmaps, faces, lodIndex = -1, currentFace, destTypeMustBeFilterable = true) {
		...
		if (info.sphericalPolynomial) sphericalPolynomialFaces = [];
		...
		if (computeFormats) internalCompressedFormat = engine._getRGBABufferInternalSizedFormat(info.textureType);
		mipmapCount = 1;
		if (header[off_flags] & DDSD_MIPMAPCOUNT && loadMipmaps !== false) mipmapCount = Math.max(1, header[off_mipmapCount]);
					...
					if (!info.isCompressed && info.isFourCC) {
						...
						if (engine._badOS || engine._badDesktopOS || !caps.textureHalfFloat && !caps.textureFloat) {
							if (bpp === 128) {
								...
								if (sphericalPolynomialFaces && i == 0) sphericalPolynomialFaces.push(DDSTools._GetFloatRGBAArrayBuffer(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i));
							} else if (bpp === 64) {
								floatArray = DDSTools._GetHalfFloatAsUIntRGBAArrayBuffer(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i);
								if (sphericalPolynomialFaces && i == 0) sphericalPolynomialFaces.push(DDSTools._GetHalfFloatAsFloatRGBAArrayBuffer(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i));
							}
							texture.type = 0;
						} else {
							...
							if (sphericalPolynomialFaces && i == 0) sphericalPolynomialFaces.push(dataGetterPolynomial ? dataGetterPolynomial(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i) : floatArray);
						}
						if (floatArray) engine._uploadDataToTextureDirectly(texture, floatArray, face, i);
					} else if (info.isRGB) {
						texture.type = 0;
						if (bpp === 24) {
							texture.format = 4;
						...
						dataLength = Math.floor((width + unpackAlignment - 1) / unpackAlignment) * unpackAlignment * (height - 1) + unpaddedRowSize;
		...
		if (sphericalPolynomialFaces && sphericalPolynomialFaces.length > 0) info.sphericalPolynomial = CubeMapToSphericalPolynomialTools.ConvertCubeMapToSphericalPolynomial({
		...
		else info.sphericalPolynomial = void 0;
	}
};
/**
...
//#endregion
export { dds_pure_exports as n, DDSTools as t };
