import { a as Hermite, u as Lerp } from "./math.scalar.functions-DvvDSyEy.js";
import { n as Observable } from "./observable.pure-COgYOPLf.js";
import { a as Vector2, i as TmpVectors, n as Quaternion, o as Vector3, t as Matrix } from "./math.vector.pure-BrntvQ1d.js";
...
import { i as _WarnImport } from "./devTools-B-EiaEIm.js";
import { C as __esDecorate, i as serialize, w as __runInitializers } from "./decorators-BbBwhhmu.js";
import { n as SerializationHelper, t as Size } from "./math.size-Bg8qa66I.js";
import { t as WebRequest } from "./webRequest-e6FlQ4_M.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/animationRange.js
/**
* Represents the range of an animation
*/
var AnimationRange = class AnimationRange {
	/**
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/node.js
/** @internal */
...
var Node = (() => {
	...
	return _a = class Node {
		...
		*/
		static Construct(type, name, scene, options) {
			...
			if (!constructorFunc) return null;
		...
		*/
		get doNotSerialize() {
			if (this._nodeDataStorage._doNotSerialize) return true;
			if (this._parentNode) return this._parentNode.doNotSerialize;
			return false;
		}
		...
		*/
		set parent(parent) {
			if (this._parentNode === parent) return;
			...
			if (this._parentNode && this._parentNode._children !== void 0 && this._parentNode._children !== null) {
				...
				if (index !== -1) this._parentNode._children.splice(index, 1);
				if (!parent && !this._nodeDataStorage._isDisposed) this._addToSceneRootNodes();
			}
			this._parentNode = parent;
			this._isDirty = true;
			if (this._parentNode) {
				if (this._parentNode._children === void 0 || this._parentNode._children === null) this._parentNode._children = new Array();
				this._parentNode._children.push(this);
				if (!previousParentNode) this._removeFromSceneRootNodes();
			}
		...
		*/
		get isVisible() {
			if (this.inheritVisibility && this._parentNode && !this._parentNode.isVisible) return false;
		...
		*/
		get animationPropertiesOverride() {
			if (!this._animationPropertiesOverride) return this._scene.animationPropertiesOverride;
		...
		*/
		constructor(name, scene = null, isPure = true) {
			...
			*/
			...
			this._initCache();
			if (isPure) this._addToSceneRootNodes();
		}
		/**
		...
		*/
		addBehavior(behavior, attachImmediately = false) {
			if (this._behaviors.indexOf(behavior) !== -1) return this;
			behavior.init();
			if (this._scene.isLoading && !attachImmediately) this._scene.onDataLoadedObservable.addOnce(() => {
				if (this._behaviors.includes(behavior)) behavior.attach(this);
			});
			else behavior.attach(this);
		...
		*/
		getWorldMatrix() {
			if (this._currentRenderId !== this._scene.getRenderId()) this.computeWorldMatrix();
		...
		*/
		...
		_markSyncedWithParent() {
			if (this._parentNode) this._parentUpdateId = this._parentNode._childUpdateId;
		}
		/** @internal */
		isSynchronizedWithParent() {
			if (!this._parentNode) return true;
			if (this._parentNode._isDirty || this._parentUpdateId !== this._parentNode._childUpdateId) return false;
			return this._parentNode.isSynchronized();
		}
		/** @internal */
		isSynchronized() {
			if (this._parentNode && !this.isSynchronizedWithParent()) return false;
		...
		*/
		isEnabled(checkAncestors = true) {
			if (checkAncestors === false) return this._nodeDataStorage._isEnabled;
			if (!this._nodeDataStorage._isEnabled) return false;
		...
		*/
		...
		_syncParentEnabledState() {
			...
			if (this._children) for (const c of this._children) c._syncParentEnabledState();
			if (notify) {
				const isEnabled = this.isEnabled();
				if (isEnabled !== wasEnabled) this._nodeDataStorage._onEffectiveEnabledStateChangedObservable.notifyObservers(isEnabled);
			}
		}
		/**
		...
		*/
		setEnabled(value) {
			if (this._nodeDataStorage._isEnabled === value) return;
			...
			if (notifyEffective) {
				const isEnabled = this.isEnabled();
				if (isEnabled !== wasEffectivelyEnabled) this._nodeDataStorage._onEffectiveEnabledStateChangedObservable.notifyObservers(isEnabled);
			}
		}
		/**
		...
		*/
		_getDescendants(results, directDescendantsOnly = false, predicate) {
			...
			for (let index = 0; index < this._children.length; index++) {
				...
				if (!directDescendantsOnly) item._getDescendants(results, false, predicate);
			}
		}
		/**
		...
		*/
		_setReady(state) {
			if (state === this._nodeDataStorage._isReady) return;
			if (!state) {
		...
		*/
		createAnimationRange(name, from, to) {
			if (!this._ranges[name]) {
				...
				for (let i = 0, nAnimations = this.animations.length; i < nAnimations; i++) if (this.animations[i]) this.animations[i].createRange(name, from, to);
			}
		}
		/**
		...
		*/
		deleteAnimationRange(name, deleteFrames = true) {
			for (let i = 0, nAnimations = this.animations.length; i < nAnimations; i++) if (this.animations[i]) this.animations[i].deleteRange(name, deleteFrames);
		...
		*/
		clone(name, newParent, doNotCloneChildren) {
			...
			if (newParent) result.parent = newParent;
		...
		*/
		getAnimationRanges() {
			...
			let name;
			for (name in this._ranges) animationRanges.push(this._ranges[name]);
		...
		*/
		computeWorldMatrix(_force) {
			if (!this._worldMatrix) this._worldMatrix = Matrix.Identity();
			return this._worldMatrix;
		}
		/**
		...
		*/
		dispose(doNotRecurse, disposeMaterialAndTextures = false) {
			...
			if (!doNotRecurse) {
				...
				for (const node of nodes) node.dispose(doNotRecurse, disposeMaterialAndTextures);
			}
			if (!this.parent) this._removeFromSceneRootNodes();
			...
			for (const behavior of this._behaviors) behavior.detach();
		...
		*/
		static ParseAnimationRanges(node, parsedNode, _scene) {
			if (parsedNode.ranges) for (let index = 0; index < parsedNode.ranges.length; index++) {
		...
		*/
		getHierarchyBoundingVectors(includeDescendants = true, predicate = null) {
			...
			if (includeDescendants) {
				...
				for (const descendant of descendants) {
					...
					if (predicate && !predicate(childMesh)) continue;
					if (!childMesh.getBoundingInfo || childMesh.getTotalVertices() === 0) continue;
		...
		__esDecorate(null, null, _id_decorators, {
			kind: "field",
			name: "id",
			static: false,
			private: false,
			...
			metadata: _metadata
		}, _id_initializers, _id_extraInitializers);
		...
		__esDecorate(null, null, _state_decorators, {
			kind: "field",
			name: "state",
			static: false,
			private: false,
			...
			metadata: _metadata
		}, _state_initializers, _state_extraInitializers);
	...
	}, _a._NodeConstructors = {}, _a;
})();
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/animation.pure.js
...
var Animation = class Animation {
	...
	*/
	static _PrepareAnimation(name, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction) {
		let dataType = void 0;
		if (!isNaN(parseFloat(from)) && isFinite(from)) dataType = Animation.ANIMATIONTYPE_FLOAT;
		else if (from instanceof Quaternion) dataType = Animation.ANIMATIONTYPE_QUATERNION;
		else if (from instanceof Vector3) dataType = Animation.ANIMATIONTYPE_VECTOR3;
		else if (from instanceof Vector2) dataType = Animation.ANIMATIONTYPE_VECTOR2;
		else if (from instanceof Color3) dataType = Animation.ANIMATIONTYPE_COLOR3;
		else if (from instanceof Color4) dataType = Animation.ANIMATIONTYPE_COLOR4;
		else if (from instanceof Size) dataType = Animation.ANIMATIONTYPE_SIZE;
		...
		if (easingFunction !== void 0) animation.setEasingFunction(easingFunction);
		return animation;
	}
	/**
	...
	*/
	get hasRunningRuntimeAnimations() {
		for (const runtimeAnimation of this._runtimeAnimations) if (!runtimeAnimation.isStopped()) return true;
		return false;
	}
	/**
	...
	*/
	removeEvents(frame) {
		for (let index = 0; index < this._events.length; index++) if (this._events[index].frame === frame) {
	...
	*/
	createRange(name, from, to) {
		if (!this._ranges[name]) this._ranges[name] = new AnimationRange(name, from, to);
	}
	/**
	...
	*/
	deleteRange(name, deleteFrames = true) {
		...
		if (deleteFrames) {
			...
			const to = range.to;
			for (let key = this._keys.length - 1; key >= 0; key--) if (this._keys[key].frame >= from && this._keys[key].frame <= to) this._keys.splice(key, 1);
		}
		this._ranges[name] = null;
	}
	/**
	...
	*/
	getHighestFrame() {
		let ret = 0;
		for (let key = 0, nKeys = this._keys.length; key < nKeys; key++) if (ret < this._keys[key].frame) ret = this._keys[key].frame;
		return ret;
	}
	/**
	...
	*/
	_interpolate(currentFrame, state, searchClosestKeyOnly = false) {
		if (state.loopMode === Animation.ANIMATIONLOOPMODE_CONSTANT && state.repeatCount > 0) return state.highLimitValue.clone ? state.highLimitValue.clone() : state.highLimitValue;
		const keys = this._keys;
		let key;
		if (!this._coreAnimation) {
			const keysLength = keys.length;
			key = state.key;
			while (key >= 0 && currentFrame < keys[key].frame) --key;
			while (key + 1 <= keysLength - 1 && currentFrame >= keys[key + 1].frame) ++key;
			state.key = key;
			if (key < 0) return searchClosestKeyOnly ? void 0 : this._getKeyValue(keys[0].value);
			else if (key + 1 > keysLength - 1) return searchClosestKeyOnly ? void 0 : this._getKeyValue(keys[keysLength - 1].value);
			this._key = key;
		} else key = this._coreAnimation._key;
		const startKey = keys[key];
		const endKey = keys[key + 1];
		if (searchClosestKeyOnly && (currentFrame === startKey.frame || currentFrame === endKey.frame)) return;
		const startValue = this._getKeyValue(startKey.value);
		const endValue = this._getKeyValue(endKey.value);
		if (startKey.interpolation === 1) {
			if (endKey.frame > currentFrame) return startValue;
		...
		switch (this.dataType) {
			case Animation.ANIMATIONTYPE_FLOAT: {
				...
				switch (state.loopMode) {
					...
					case Animation.ANIMATIONLOOPMODE_YOYO: return floatValue;
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return (state.offsetValue ?? 0) * state.repeatCount + floatValue;
				}
				break;
			}
			case Animation.ANIMATIONTYPE_QUATERNION: {
				...
				switch (state.loopMode) {
					case Animation.ANIMATIONLOOPMODE_CYCLE:
					case Animation.ANIMATIONLOOPMODE_CONSTANT:
					case Animation.ANIMATIONLOOPMODE_YOYO: return quatValue;
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return quatValue.addInPlace((state.offsetValue || _StaticOffsetValueQuaternion).scale(state.repeatCount));
				}
				return quatValue;
			}
			case Animation.ANIMATIONTYPE_VECTOR3: {
				...
				switch (state.loopMode) {
					case Animation.ANIMATIONLOOPMODE_CYCLE:
					case Animation.ANIMATIONLOOPMODE_CONSTANT:
					case Animation.ANIMATIONLOOPMODE_YOYO: return vec3Value;
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return vec3Value.add((state.offsetValue || _StaticOffsetValueVector3).scale(state.repeatCount));
				}
				break;
			}
			case Animation.ANIMATIONTYPE_VECTOR2: {
				...
				switch (state.loopMode) {
					case Animation.ANIMATIONLOOPMODE_CYCLE:
					case Animation.ANIMATIONLOOPMODE_CONSTANT:
					case Animation.ANIMATIONLOOPMODE_YOYO: return vec2Value;
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return vec2Value.add((state.offsetValue || _StaticOffsetValueVector2).scale(state.repeatCount));
				}
				break;
			}
			case Animation.ANIMATIONTYPE_SIZE:
				switch (state.loopMode) {
					case Animation.ANIMATIONLOOPMODE_CYCLE:
					case Animation.ANIMATIONLOOPMODE_CONSTANT:
					case Animation.ANIMATIONLOOPMODE_YOYO: return this.sizeInterpolateFunction(startValue, endValue, gradient);
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return this.sizeInterpolateFunction(startValue, endValue, gradient).add((state.offsetValue || _StaticOffsetValueSize).scale(state.repeatCount));
				}
				break;
			case Animation.ANIMATIONTYPE_COLOR3: {
				...
				switch (state.loopMode) {
					case Animation.ANIMATIONLOOPMODE_CYCLE:
					case Animation.ANIMATIONLOOPMODE_CONSTANT:
					case Animation.ANIMATIONLOOPMODE_YOYO: return color3Value;
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return color3Value.add((state.offsetValue || _StaticOffsetValueColor3).scale(state.repeatCount));
				}
				break;
			}
			case Animation.ANIMATIONTYPE_COLOR4: {
				...
				switch (state.loopMode) {
					case Animation.ANIMATIONLOOPMODE_CYCLE:
					case Animation.ANIMATIONLOOPMODE_CONSTANT:
					case Animation.ANIMATIONLOOPMODE_YOYO: return color4Value;
					case Animation.ANIMATIONLOOPMODE_RELATIVE:
					case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return color4Value.add((state.offsetValue || _StaticOffsetValueColor4).scale(state.repeatCount));
				}
				break;
			}
			case Animation.ANIMATIONTYPE_MATRIX: switch (state.loopMode) {
				case Animation.ANIMATIONLOOPMODE_CYCLE:
				case Animation.ANIMATIONLOOPMODE_CONSTANT:
				case Animation.ANIMATIONLOOPMODE_YOYO:
					if (Animation.AllowMatricesInterpolation) return this.matrixInterpolateFunction(startValue, endValue, gradient, state.workValue);
					return startValue;
				case Animation.ANIMATIONLOOPMODE_RELATIVE:
				case Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT: return startValue;
			}
		}
		return 0;
	}
	...
	*/
	clone(cloneKeys = false) {
		...
		if (this._keys) clone.setKeys(this._keys, false, cloneKeys);
		if (this._ranges) {
			clone._ranges = {};
			for (const name in this._ranges) {
				const range = this._ranges[name];
	...
	*/
	setKeys(values, dontClone = false, cloneKeys = false) {
		if (!dontClone) {
			...
			if (cloneKeys) for (let i = 0; i < this._keys.length; i++) {
	...
	*/
	createKeyForFrame(frame) {
		...
		if (!value) return this._keys[EvaluateAnimationState.key].frame === frame ? EvaluateAnimationState.key : EvaluateAnimationState.key + 1;
		const newKey = {
			frame,
		...
		for (let index = 0; index < keys.length; index++) {
			...
			switch (dataType) {
				case Animation.ANIMATIONTYPE_FLOAT:
					...
					if (animationKey.inTangent !== void 0) key.values.push(animationKey.inTangent);
					if (animationKey.outTangent !== void 0) {
						if (animationKey.inTangent === void 0) key.values.push(void 0);
						key.values.push(animationKey.outTangent);
					}
					if (animationKey.interpolation !== void 0) {
						if (animationKey.inTangent === void 0) key.values.push(void 0);
						if (animationKey.outTangent === void 0) key.values.push(void 0);
				...
				case Animation.ANIMATIONTYPE_COLOR4:
					...
					if (animationKey.inTangent != void 0) key.values.push(animationKey.inTangent.asArray());
					if (animationKey.outTangent != void 0) {
						if (animationKey.inTangent === void 0) key.values.push(void 0);
	...
	*/
	static _UniversalLerp(left, right, amount) {
		...
		if (constructor.Lerp) return constructor.Lerp(left, right, amount);
		else if (constructor.Slerp) return constructor.Slerp(left, right, amount);
		else if (left.toFixed) return left * (1 - amount) + amount * right;
		else return right;
	}
};
...
function AnimationMakeAnimationAdditive(sourceAnimation, referenceFrameOrOptions, range, cloneOriginal = false, clonedName) {
	...
	if (!keys.length) return animation;
	...
	if (from !== firstKey.frame) startIndex = animation.createKeyForFrame(from);
	if (to !== lastKey.frame) endIndex = animation.createKeyForFrame(to);
	...
	if (animation.dataType === Animation.ANIMATIONTYPE_QUATERNION) valueStore.referenceValue.normalize().conjugateInPlace();
	...
	for (let index = startIndex; index <= endIndex; index++) {
		...
		if (index && animation.dataType !== Animation.ANIMATIONTYPE_FLOAT && key.value === firstKey.value) continue;
		switch (animation.dataType) {
			...
			case Animation.ANIMATIONTYPE_SIZE:
				...
				break;
			default: key.value -= valueStore.referenceValue;
		}
	}
	if (clippedKeys) animation.setKeys(clippedKeys, true);
	return animation;
}
/**
...
function AnimationCreateAndStartAnimation(name, target, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd, scene) {
	...
	if (target.getScene) scene = target.getScene();
...
function AnimationCreateAndStartHierarchyAnimation(name, node, directDescendantsOnly, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction, onAnimationEnd) {
	const animation = Animation._PrepareAnimation(name, targetProperty, framePerSecond, totalFrame, from, to, loopMode, easingFunction);
	if (!animation) return null;
	return node.getScene().beginDirectHierarchyAnimation(node, directDescendantsOnly, [animation], 0, totalFrame, animation.loopMode === 1, 1, onAnimationEnd);
}
/**
...
function AnimationTransitionTo(property, targetValue, host, scene, frameRate, transition, duration, onAnimationEnd = null, stopCurrent = true, customKeys) {
	...
	if (!host.animations) host.animations = [];
	host.animations.push(transition);
	return scene.beginAnimation(host, 0, endFrame, false, 1, onAnimationEnd ?? void 0, void 0, stopCurrent);
}
/**
...
function AnimationParse(parsedAnimation) {
	...
	let data;
	let index;
	if (parsedAnimation.enableBlending) animation.enableBlending = parsedAnimation.enableBlending;
	if (parsedAnimation.blendingSpeed) animation.blendingSpeed = parsedAnimation.blendingSpeed;
	for (index = 0; index < parsedAnimation.keys.length; index++) {
		...
		switch (dataType) {
			case Animation.ANIMATIONTYPE_FLOAT:
				data = key.values[0];
				if (key.values.length >= 2) inTangent = key.values[1];
				if (key.values.length >= 3) outTangent = key.values[2];
				if (key.values.length >= 4) interpolation = key.values[3];
				break;
			case Animation.ANIMATIONTYPE_QUATERNION:
				data = Quaternion.FromArray(key.values);
				if (key.values.length >= 8) {
					const _inTangent = Quaternion.FromArray(key.values.slice(4, 8));
					if (!_inTangent.equals(Quaternion.Zero())) inTangent = _inTangent;
				}
				if (key.values.length >= 12) {
					const _outTangent = Quaternion.FromArray(key.values.slice(8, 12));
					if (!_outTangent.equals(Quaternion.Zero())) outTangent = _outTangent;
				}
				if (key.values.length >= 13) interpolation = key.values[12];
				break;
			case Animation.ANIMATIONTYPE_MATRIX:
				data = Matrix.FromArray(key.values);
				if (key.values.length >= 17) interpolation = key.values[16];
				break;
			case Animation.ANIMATIONTYPE_COLOR3:
				data = Color3.FromArray(key.values);
				if (key.values[3]) inTangent = Color3.FromArray(key.values[3]);
				if (key.values[4]) outTangent = Color3.FromArray(key.values[4]);
				if (key.values[5]) interpolation = key.values[5];
				break;
			case Animation.ANIMATIONTYPE_COLOR4:
				data = Color4.FromArray(key.values);
				if (key.values[4]) inTangent = Color4.FromArray(key.values[4]);
				if (key.values[5]) outTangent = Color4.FromArray(key.values[5]);
				if (key.values[6]) interpolation = key.values[6];
				break;
			case Animation.ANIMATIONTYPE_VECTOR3:
			default:
				data = Vector3.FromArray(key.values);
				if (key.values[3]) inTangent = Vector3.FromArray(key.values[3]);
				if (key.values[4]) outTangent = Vector3.FromArray(key.values[4]);
				if (key.values[5]) interpolation = key.values[5];
		}
		const keyData = {};
		keyData.frame = key.frame;
		keyData.value = data;
		if (inTangent != void 0) keyData.inTangent = inTangent;
		if (outTangent != void 0) keyData.outTangent = outTangent;
		if (interpolation != void 0) keyData.interpolation = interpolation;
		keys.push(keyData);
	}
	animation.setKeys(keys);
	if (parsedAnimation.ranges) for (index = 0; index < parsedAnimation.ranges.length; index++) {
		...
		request.addEventListener("readystatechange", () => {
			if (request.readyState == 4) {
				if (request.status == 200) {
					...
					if (serializationObject.animations) serializationObject = serializationObject.animations;
					if (serializationObject.length) {
						const output = [];
						for (const serializedAnimation of serializationObject) output.push(AnimationParse(serializedAnimation));
						resolve(output);
					} else {
				...
				} else reject("Unable to load the animation");
			}
		});
			...
			if (request.readyState == 4) {
				if (request.status == 200) {
					...
					if (snippet.animations) {
						...
						const output = AnimationParse(JSON.parse(snippet.animation));
						output.snippetId = snippetId;
						resolve(output);
					}
				} else reject("Unable to load the snippet " + snippetId);
			}
		});
...
//#endregion
export { _StaticOffsetValueSize as _, AnimationCreateAnimation as a, Node as b, AnimationMakeAnimationAdditive as c, AnimationParseFromSnippetAsync as d, AnimationTransitionTo as f, _StaticOffsetValueQuaternion as g, _StaticOffsetValueColor4 as h, AnimationCreateAndStartHierarchyAnimation as i, AnimationParse as l, _StaticOffsetValueColor3 as m, AnimationAppendSerializedAnimations as n, AnimationCreateFromSnippetAsync as o, RegisterAnimation as p, AnimationCreateAndStartAnimation as r, AnimationCreateMergeAndStartAnimation as s, Animation as t, AnimationParseFromFileAsync as u, _StaticOffsetValueVector2 as v, AnimationRange as x, _StaticOffsetValueVector3 as y };
