...
import { i as FlowGraphConnection, t as FlowGraphBlock } from "./flowGraphBlock-CCYOelAH.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/flowGraphSignalConnection.pure.js
	...
	*/
	_activateSignal(context) {
		...
		if (this.connectionType === 0) {
			if (context._shouldBreak(this._ownerBlock, this)) return;
		...
		} else for (const connectedPoint of this._connectedPoint) connectedPoint._activateSignal(context);
	}
};
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/flowGraphExecutionBlock.js
/**
	...
	*/
	deserialize(serializationObject) {
		for (let i = 0; i < serializationObject.signalInputs.length; i++) {
			...
			if (signalInput) signalInput.deserialize(serializationObject.signalInputs[i]);
			else throw new Error("Could not find signal input with name " + serializationObject.signalInputs[i].name + " in block " + serializationObject.className);
		}
		for (let i = 0; i < serializationObject.signalOutputs.length; i++) {
			const signalOutput = this.getSignalOutput(serializationObject.signalOutputs[i].name);
			if (signalOutput) signalOutput.deserialize(serializationObject.signalOutputs[i]);
			else throw new Error("Could not find signal output with name " + serializationObject.signalOutputs[i].name + " in block " + serializationObject.className);
		}
	}
	/**
...
//#endregion
export { FlowGraphSignalConnection as n, RegisterFlowGraphSignalConnection as r, FlowGraphExecutionBlock as t };
