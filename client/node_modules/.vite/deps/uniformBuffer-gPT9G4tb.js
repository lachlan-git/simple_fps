...
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Materials/uniformBuffer.js
/**
...
*/
var UniformBuffer = class UniformBuffer {
	/**
	...
	*/
	addUniform(name, size, arraySize = 0) {
		if (arraySize > 0 && typeof size === "number") this._uniformArraySizes[name] = {
			strideSize: size,
			arraySize
		};
		if (this._uniformLocations[name] !== void 0) return;
		this._uniformNames.push(name);
		if (this._noUBO) return;
		let data;
		if (arraySize > 0) {
			if (size instanceof Array) throw "addUniform should not be use with Array in UBO: " + name;
			this._fillAlignment(4);
			if (size == 16) size = size * arraySize;
			else {
				const totalPadding = (4 - size) * arraySize;
		...
		for (let i = 0; i < size; i++) this._data.push(data[i]);
		this._needSync = true;
	}
	/**
	...
	*/
	...
	/** @internal */
	_rebuild() {
		if (this._noUBO || !this._bufferData) return;
		if (this._dynamic) this._buffer = this._engine.createDynamicUniformBuffer(this._bufferData, this._name + "_UniformList:" + this._getNamesDebug());
		else this._buffer = this._engine.createUniformBuffer(this._bufferData, this._name + "_UniformList:" + this._getNamesDebug());
	...
	_checkNewFrame() {
		if (this._trackUBOsInFrame && this._currentFrameId !== this._engine.frameId) {
			...
			if (this._buffers && this._buffers.length > 0) {
				if (this._buffers.length === 1) this._needSync = !this._bufferUpdatedLastFrame;
				else this._needSync = this._bufferIndex !== 0;
				this._bufferIndex = 0;
				this._buffer = this._buffers[this._bufferIndex][0];
			} else this._bufferIndex = -1;
		}
	}
	/**
	...
	*/
	updateUniform(uniformName, data, size) {
		...
		if (!this._dynamic) {
			let changed = false;
			for (let i = 0; i < size; i++) if (size === 16 && !this._engine._features.uniformBufferHardCheckMatrix || this._bufferData[location + i] !== Math.fround(data[i])) {
				changed = true;
				if (this._createBufferOnWrite) this._createNewBuffer();
				this._bufferData[location + i] = data[i];
			}
			this._needSync = this._needSync || changed;
		} else for (let i = 0; i < size; i++) this._bufferData[location + i] = data[i];
	}
	/**
	* Updates the value of an uniform. The `update` method must be called afterwards to make it effective in the GPU.
	* @param uniformName Define the name of the uniform, as used in the uniform block in the shader.
	...
	*/
		...
		if (!this._dynamic) {
			...
			for (let i = 0; i < size; i++) {
				...
				if (countToFour === arraySizes.strideSize) {
					for (; countToFour < 4; countToFour++) this._bufferData[location + baseStride * 4 + countToFour] = 0;
					countToFour = 0;
					baseStride++;
				}
			}
	...
	_updateFloatArrayForEffect(name, array, suffix = "") {
		switch (this._uniformArraySizes[name]?.strideSize) {
			...
			case 4:
				...
				break;
			default: this._currentEffect.setFloatArray(name + suffix, array);
		}
	}
	...
	_updateMatrixForUniform(name, mat) {
		if (this._cacheMatrix(name, mat)) this.updateUniform(name, mat.asArray(), 16);
	}
	...
	*/
	bindUniformBuffer() {
		if (!this._noUBO && this._buffer && this._currentEffect) this._currentEffect.bindUniformBuffer(this._buffer, this._currentEffectName);
	}
	/**
	...
	*/
	setDataBuffer(dataBuffer) {
		if (!this._buffers) return this._buffer === dataBuffer;
		for (let b = 0; b < this._buffers.length; ++b) if (this._buffers[b][0] === dataBuffer) {
			this._bufferIndex = b;
			this._buffer = dataBuffer;
			this._createBufferOnWrite = false;
			this._currentEffect = void 0;
			if (this._buffers.length > 1 && this._buffers[b][1]) this._bufferData.set(this._buffers[b][1]);
			this._valueCache = {};
			this._currentFrameId = this._engine.frameId;
			return true;
		}
	...
	*/
	dispose() {
		...
		if (this._trackUBOsInFrame && this._buffers) for (let i = 0; i < this._buffers.length; ++i) {
			const buffer = this._buffers[i][0];
			this._engine._releaseBuffer(buffer);
		}
		else if (this._buffer && this._engine._releaseBuffer(this._buffer)) this._buffer = null;
	}
};
