...
import { M as GetTypeByteLength, j as GetFloatData, k as EnumerateFloatValues } from "./tools.pure-DiR-ZH3i.js";
import { d as DataBuffer, t as ThinEngine } from "./thinEngine.pure-Dr2uaZKU.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Engines/Extensions/engine.dynamicBuffer.pure.js
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Buffers/buffer.pure.js
/** This file must only contain pure code and pure imports */
/**
...
var Buffer = class {
	...
	*/
	constructor(engine, data, updatable, stride = 0, postponeInternalCreation = false, instanced = false, useBytes = false, divisor, label) {
		...
		if (engine && engine.getScene) this._engine = engine.getScene().getEngine();
	...
	create(data = null) {
		...
		if (!this._buffer) {
			if (this._updatable) {
				...
				this._data = data;
			} else this._buffer = this._engine.createVertexBuffer(data, void 0, this._label);
	...
	*/
	updateDirectly(data, offset, vertexCount, useBytes = false) {
		...
		if (this._updatable) {
			...
			if (offset === 0 && vertexCount === void 0) this._data = data;
...
*/
var VertexBuffer = class VertexBuffer {
	/**
	* Gets a boolean indicating if the Buffer is disposed
	*/
	get isDisposed() {
	...
	*/
	get _maxVerticesCount() {
		...
		if (!data) return 0;
		if (Array.isArray(data)) return data.length / (this.byteStride / 4) - this.byteOffset / 4;
	...
	constructor(engine, data, kind, updatableOrOptions, postponeInternalCreation, stride, instanced, offset, size, type, normalized = false, useBytes = false, divisor = 1, takeBufferOwnership = false) {
		...
		} else updatable = !!updatableOrOptions;
	...
	static _GetTypeHashIndex(type) {
		switch (type) {
			case VertexBuffer.BYTE: return 0;
			case VertexBuffer.UNSIGNED_BYTE: return 1;
			case VertexBuffer.SHORT: return 2;
			case VertexBuffer.UNSIGNED_SHORT: return 3;
			case VertexBuffer.INT: return 4;
			case VertexBuffer.UNSIGNED_INT: return 5;
			case VertexBuffer.FLOAT: return 6;
			case VertexBuffer.HALF_FLOAT: return 7;
			default: throw new Error(`Invalid vertex buffer type '${type}'`);
		}
	}
	_computeHashCode() {
		this.hashCode = (VertexBuffer._GetTypeHashIndex(this.type) << 0) + ((this.normalized ? 1 : 0) << 3) + (this._size - 1 << 4) + ((this._instanced ? 1 : 0) << 6) + (this.byteStride << 12);
	}
	/** @internal */
	_rebuild() {
	...
	*/
	dispose() {
		if (this._ownsBuffer) this._buffer.dispose();
		this._isDisposed = true;
	}
	/**
	...
	*/
	forEach(count, callback) {
		EnumerateFloatValues(this._buffer.getData(), this.byteOffset, this.byteStride, this._size, this.type, count, this.normalized, (values, index) => {
			for (let i = 0; i < this._size; i++) callback(values[i], index + i);
		});
	}
	/** @internal */
	_alignBuffer() {}
...
function VertexBufferDeduceStride(kind) {
	switch (kind) {
		...
		case VertexBuffer.UV6Kind: return 2;
		case VertexBuffer.NormalKind:
		case VertexBuffer.PositionKind: return 3;
		...
		case VertexBuffer.TangentKind: return 4;
...
function VertexBufferGetDataType(data) {
	if (data instanceof Int8Array) return VertexBuffer.BYTE;
	else if (data instanceof Uint8Array) return VertexBuffer.UNSIGNED_BYTE;
	else if (data instanceof Int16Array) return VertexBuffer.SHORT;
	else if (data instanceof Uint16Array) return VertexBuffer.UNSIGNED_SHORT;
	else if (data instanceof Int32Array) return VertexBuffer.INT;
	else if (data instanceof Uint32Array) return VertexBuffer.UNSIGNED_INT;
	else return VertexBuffer.FLOAT;
}
/**
...
function VertexBufferForEach(data, byteOffset, byteStride, componentCount, componentType, count, normalized, callback) {
	EnumerateFloatValues(data, byteOffset, byteStride, componentCount, componentType, count, normalized, (values, index) => {
		for (let componentIndex = 0; componentIndex < componentCount; componentIndex++) callback(values[componentIndex], index + componentIndex);
	});
}
/**
...
//#endregion
export { VertexBufferForEach as a, VertexBufferGetTypeByteLength as c, VertexBufferDeduceStride as i, RegisterEngineDynamicBuffer as l, RegisterVertexBuffer as n, VertexBufferGetDataType as o, VertexBuffer as r, VertexBufferGetFloatData as s, Buffer as t };
