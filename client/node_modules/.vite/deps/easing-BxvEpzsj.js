import { t as Clamp, w as WithinEpsilon } from "./math.scalar.functions-DvvDSyEy.js";
import { a as Vector2, n as Quaternion, o as Vector3, t as Matrix } from "./math.vector.pure-BrntvQ1d.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Maths/math.path.js
/**
	...
	*/
	static Interpolate(t, x1, y1, x2, y2) {
		...
		for (let i = 0; i < 5; i++) {
			...
			const x = f0 * (refinedT2 * refinedT) + f1 * refinedT2 + f2 * refinedT;
...
var Path2 = class Path2 {
	...
	*/
	addArcTo(midX, midY, endX, endY, numberOfSegments = 36) {
		...
		const arc = new Arc2(startPoint, new Vector2(midX, midY), new Vector2(endX, endY));
		let increment = arc.angle.radians() / numberOfSegments;
		if (arc.orientation === 0) increment *= -1;
	...
	*/
	addQuadraticCurveTo(controlX, controlY, endX, endY, numberOfSegments = 36) {
		...
		const equation = (t, val0, val1, val2) => {
			return (1 - t) * (1 - t) * val0 + 2 * t * (1 - t) * val1 + t * t * val2;
		};
		const startPoint = this._points[this._points.length - 1];
		for (let i = 0; i <= numberOfSegments; i++) {
	...
	*/
	addBezierCurveTo(originTangentX, originTangentY, destinationTangentX, destinationTangentY, endX, endY, numberOfSegments = 36) {
		...
		const equation = (t, val0, val1, val2, val3) => {
			return (1 - t) * (1 - t) * (1 - t) * val0 + 3 * t * (1 - t) * (1 - t) * val1 + 3 * t * t * (1 - t) * val2 + t * t * t * val3;
		};
		const startPoint = this._points[this._points.length - 1];
		for (let i = 0; i <= numberOfSegments; i++) {
			const step = i / numberOfSegments;
	...
	isPointInside(point) {
		...
		for (let p = count - 1, q = 0; q < count; p = q++) {
			...
			if (Math.abs(edgeDy) > Number.EPSILON) {
				...
				if (point.y < edgeLow.y || point.y > edgeHigh.y) continue;
				if (point.y === edgeLow.y && point.x === edgeLow.x) return true;
				else {
				...
				if (point.y !== edgeLow.y) continue;
				if (edgeHigh.x <= point.x && point.x <= edgeLow.x || edgeLow.x <= point.x && point.x <= edgeHigh.x) return true;
			}
		}
		return isInside;
	}
	...
	*/
	area() {
		...
		let value = 0;
		for (let p = n - 1, q = 0; q < n; p = q++) value += this._points[p].x * this._points[q].y - this._points[q].x * this._points[p].y;
		return value * .5;
	}
	/**
	...
	*/
	getPointAtLengthPosition(normalizedLengthPosition) {
		if (normalizedLengthPosition < 0 || normalizedLengthPosition > 1) return Vector2.Zero();
		const lengthPosition = normalizedLengthPosition * this.length();
		let previousOffset = 0;
		for (let i = 0; i < this._points.length; i++) {
			const j = (i + 1) % this._points.length;
			const a = this._points[i];
			const bToA = this._points[j].subtract(a);
...
var Path3D = class Path3D {
	...
	*/
	constructor(path, firstNormal = null, raw, alignTangentsWithPath = false) {
		...
		for (let p = 0; p < path.length; p++) this._curve[p] = path[p].clone();
		this._raw = raw || false;
	...
	*/
	slice(start = 0, end = 1) {
		...
		if (end !== 1 || start === 1) slicePoints.push(endPoint);
	...
	*/
	...
	_compute(firstNormal, alignTangentsWithPath = false) {
		...
		for (let i = 1; i < l; i++) {
			...
			if (!this._raw) {
				...
				} else this._normals[i].normalize();
			}
	...
	_normalVector(vt, va) {
		...
		if (va === void 0 || va === null) {
			let point;
			if (!WithinEpsilon(Math.abs(vt.y) / tgl, 1, .001)) point = new Vector3(0, -1, 0);
			else if (!WithinEpsilon(Math.abs(vt.x) / tgl, 1, .001)) point = new Vector3(1, 0, 0);
			else if (!WithinEpsilon(Math.abs(vt.z) / tgl, 1, .001)) point = new Vector3(0, 0, 1);
	...
	*/
	_updatePointAtData(position, interpolateTNB = false) {
		if (this._pointAtData.id === position) {
			if (!this._pointAtData.interpolateReady) this._updateInterpolationMatrix();
			return this._pointAtData;
		} else this._pointAtData.id = position;
		const curvePoints = this.getPoints();
		if (position <= 0) return this._setPointAtData(0, 0, curvePoints[0], 0, interpolateTNB);
		else if (position >= 1) return this._setPointAtData(1, 1, curvePoints[curvePoints.length - 1], curvePoints.length - 1, interpolateTNB);
		...
		for (let i = 1; i < curvePoints.length; i++) {
			...
			if (currentLength === targetLength) return this._setPointAtData(position, 1, currentPoint, i, interpolateTNB);
			else if (currentLength > targetLength) {
				const diff = (currentLength - targetLength) / distance;
	...
	*/
	_setPointAtData(position, subPosition, point, parentIndex, interpolateTNB) {
		...
		if (interpolateTNB) this._updateInterpolationMatrix();
		return this._pointAtData;
	}
	/**
	...
	*/
	_updateInterpolationMatrix() {
		...
		if (parentIndex !== this._tangents.length - 1) {
			...
			Quaternion.Slerp(quatFrom, quatTo, this._pointAtData.subPosition).toRotationMatrix(this._pointAtData.interpolationMatrix);
		}
	}
};
/**
...
var Curve3 = class Curve3 {
	...
	*/
	static CreateQuadraticBezier(v0, v1, v2, nbPoints) {
		...
		for (let i = 0; i <= nbPoints; i++) bez.push(new Vector3(equation(i / nbPoints, v0.x, v1.x, v2.x), equation(i / nbPoints, v0.y, v1.y, v2.y), equation(i / nbPoints, v0.z, v1.z, v2.z)));
	...
	*/
	static CreateCubicBezier(v0, v1, v2, v3, nbPoints) {
		...
		for (let i = 0; i <= nbPoints; i++) bez.push(new Vector3(equation(i / nbPoints, v0.x, v1.x, v2.x, v3.x), equation(i / nbPoints, v0.y, v1.y, v2.y, v3.y), equation(i / nbPoints, v0.z, v1.z, v2.z, v3.z)));
		return new Curve3(bez);
	}
	/**
	...
	*/
	static CreateHermiteSpline(p1, t1, p2, t2, nSeg) {
		const hermite = [];
		const step = 1 / nSeg;
		for (let i = 0; i <= nSeg; i++) hermite.push(Vector3.Hermite(p1, t1, p2, t2, i * step));
	...
	*/
	static ArcThru3Points(first, second, third, steps = 32, closed = false, fullCircle = false) {
		...
		if (len4 < Math.pow(10, -8)) return new Curve3(arc);
		...
		const radius = .5 * len1 * len2 * len3 / len4;
		const dot1 = Vector3.Dot(vec1, vec3);
		const dot2 = Vector3.Dot(vec1, vec2);
		const dot3 = Vector3.Dot(vec2, vec3);
		const a = -.5 * len2Sq * dot1 / len4Sq;
		const b = -.5 * len3Sq * dot2 / len4Sq;
		const c = -.5 * len1Sq * dot3 / len4Sq;
		const center = first.scale(a).add(second.scale(b)).add(third.scale(c));
		const xAxis = first.subtract(center).normalize();
		const yAxis = Vector3.Cross(zAxis, xAxis).normalize();
		if (fullCircle) {
			const dStep = 2 * Math.PI / steps;
			for (let theta = 0; theta <= 2 * Math.PI; theta += dStep) arc.push(center.add(xAxis.scale(radius * Math.cos(theta)).add(yAxis.scale(radius * Math.sin(theta)))));
			arc.push(first);
		} else {
			const dStep = 1 / steps;
			let theta = 0;
	...
	*/
	continue(curve) {
		...
		for (let i = 1; i < curvePoints.length; i++) continuedPoints.push(curvePoints[i].subtract(curvePoints[0]).add(lastPoint));
		return new Curve3(continuedPoints);
	}
	_computeLength(path) {
		let l = 0;
		for (let i = 1; i < path.length; i++) l += path[i].subtract(path[i - 1]).length();
		return l;
	}
};
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/easing.js
/**
* Base class used for every default easing function.
* @see https://doc.babylonjs.com/features/featuresDeepDive/animation/advanced_animations#easing-functions
*/
var EasingFunction = class EasingFunction {
	constructor() {
	...
	*/
	ease(gradient) {
		switch (this._easingMode) {
			case EasingFunction.EASINGMODE_EASEIN: return this.easeInCore(gradient);
			case EasingFunction.EASINGMODE_EASEOUT: return 1 - this.easeInCore(1 - gradient);
		}
		if (gradient >= .5) return (1 - this.easeInCore((1 - gradient) * 2)) * .5 + .5;
...
var BounceEase = class extends EasingFunction {
	...
	*/
	easeInCore(gradient) {
		...
		if (bounciness <= 1) bounciness = 1.001;
		const num9 = Math.pow(bounciness, y);
		const num5 = 1 - bounciness;
		const num4 = (1 - num9) / num5 + num9 * .5;
		...
		const num7 = (num8 + (1 - Math.pow(bounciness, num13)) / (num5 * num4)) * .5;
...
var ElasticEase = class extends EasingFunction {
	...
	*/
	easeInCore(gradient) {
		...
		if (num === 0) num2 = gradient;
		else num2 = (Math.exp(num * gradient) - 1) / (Math.exp(num) - 1);
...
//#endregion
export { Curve3 as _, CubicEase as a, Path3D as b, ExponentialEase as c, QuarticEase as d, QuinticEase as f, BezierCurve as g, Arc2 as h, CircleEase as i, PowerEase as l, Angle as m, BezierCurveEase as n, EasingFunction as o, SineEase as p, BounceEase as r, ElasticEase as s, BackEase as t, QuadraticEase as u, Orientation as v, Path2 as y };
