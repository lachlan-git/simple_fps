...
import { a as Vector2, n as Quaternion, o as Vector3, s as Vector4, t as Matrix } from "./math.vector.pure-BrntvQ1d.js";
...
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/CustomTypes/flowGraphInteger.pure.js
/** This file must only contain pure code and pure imports */
/**
* Class that represents an integer value.
*/
var FlowGraphInteger = class FlowGraphInteger {
	constructor(value) {
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/CustomTypes/flowGraphMatrix.js
/**
...
*/
var FlowGraphMatrix2D = class FlowGraphMatrix2D {
	constructor(m = [
		1,
		0,
		0,
	...
	toArray(emptyArray = []) {
		for (let i = 0; i < 4; i++) emptyArray[i] = this._m[i];
		return emptyArray;
	}
	fromArray(array) {
		for (let i = 0; i < 4; i++) this._m[i] = array[i];
		return this;
	}
	...
	equals(other, epsilon = 0) {
		const m = this._m;
		const o = other.m;
		if (epsilon === 0) return m[0] === o[0] && m[1] === o[1] && m[2] === o[2] && m[3] === o[3];
...
* behaviour change (flat-array construction, transform, and multiply operand order).
*/
var FlowGraphMatrix3D = class FlowGraphMatrix3D {
	constructor(array = [
		1,
		0,
		0,
	...
	toArray(emptyArray = []) {
		for (let i = 0; i < 9; i++) emptyArray[i] = this._m[i];
		return emptyArray;
	}
	asArray() {
		return this.toArray();
	...
	fromArray(array) {
		for (let i = 0; i < 9; i++) this._m[i] = array[i];
		return this;
	}
	transpose() {
		const m = this._m;
	...
	equals(other, epsilon = 0) {
		const m = this._m;
		const o = other.m;
		if (epsilon === 0) return m[0] === o[0] && m[1] === o[1] && m[2] === o[2] && m[3] === o[3] && m[4] === o[4] && m[5] === o[5] && m[6] === o[6] && m[7] === o[7] && m[8] === o[8];
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/flowGraphRichTypes.pure.js
/** This file must only contain pure code and pure imports */
/**
...
function getRichTypeFromValue(value) {
	...
	switch (typeof value) {
		case "string": return RichTypeString;
		case "number": return RichTypeNumber;
		case "boolean": return RichTypeBoolean;
		case "object":
			if (anyValue.getClassName) switch (anyValue.getClassName()) {
				case "Vector2": return RichTypeVector2;
				case "Vector3": return RichTypeVector3;
				case "Vector4": return RichTypeVector4;
				case "Matrix": return RichTypeMatrix;
				case "Color3": return RichTypeColor3;
				case "Color4": return RichTypeColor4;
				case "Quaternion": return RichTypeQuaternion;
				case "FlowGraphInteger": return RichTypeFlowGraphInteger;
				case "Matrix2D": return RichTypeMatrix2D;
				case "Matrix3D": return RichTypeMatrix3D;
			}
			return RichTypeAny;
		default: return RichTypeAny;
	}
}
/**
...
function getRichTypeByAnimationType(animationType) {
	switch (animationType) {
		case 0: return RichTypeNumber;
		case 5: return RichTypeVector2;
		case 1: return RichTypeVector3;
		case 3: return RichTypeMatrix;
		case 4: return RichTypeColor3;
		case 7: return RichTypeColor4;
		case 2: return RichTypeQuaternion;
		default: return RichTypeAny;
	}
}
...
function RegisterFlowGraphRichTypes() {
	...
	RichTypeQuaternion.typeTransformer = (value) => {
		if (value.getClassName) {
			if (value.getClassName() === "Vector4") return Quaternion.FromArray(value.asArray());
			else if (value.getClassName() === "Vector3") return Quaternion.FromEulerVector(value);
			else if (value.getClassName() === "Matrix") return Quaternion.FromRotationMatrix(value);
		}
		return value;
	};
}
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/serialization.js
...
function GetSceneNodeFromSerializedReference(serializedReference, scene) {
	if (!serializedReference || !serializedReference.id && !serializedReference.name) return;
...
function ParseVector(className, value, flipHandedness = false) {
	if (className === "Vector2") return Vector2.FromArray(value);
	else if (className === "Vector3") {
		if (flipHandedness) value[2] *= -1;
		return Vector3.FromArray(value);
	} else if (className === "Vector4") return Vector4.FromArray(value);
	...
	} else if (className === "Color3") return new Color3(value[0], value[1], value[2]);
	else if (className === "Color4") return new Color4(value[0], value[1], value[2], value[3]);
	else throw new Error(`Unknown vector class name ${className}`);
}
/**
...
function defaultValueSerializationFunction(key, value, serializationObject) {
	...
	if (IsVectorClassName(className) || IsMatrixClassName(className)) serializationObject[key] = {
		value: value.asArray(),
		className
	};
	else if (className === "FlowGraphInteger") serializationObject[key] = {
		value: value.value,
		className
	};
	else if (className && (value.id || value.name)) serializationObject[key] = {
		id: value.id,
		name: value.name,
		className,
		uniqueId: value.uniqueId
	};
	else if (typeof value !== "object" || value === null) serializationObject[key] = value;
	else {
		if (key === "pathConverter") return;
		if (Object.values(value).some((v) => typeof v === "function")) return;
		try {
...
function defaultValueParseFunction(key, serializationObject, assetsContainer, scene) {
	...
	if (sceneNode) finalValue = sceneNode;
	else if (IsVectorClassName(className)) finalValue = ParseVector(className, intermediateValue.value);
	...
	} else if (className === "Matrix") finalValue = Matrix.FromArray(intermediateValue.value);
	else if (className === "Matrix2D") finalValue = new FlowGraphMatrix2D(intermediateValue.value);
	else if (className === "Matrix3D") finalValue = new FlowGraphMatrix3D(intermediateValue.value);
	else if (className === "FlowGraphInteger") finalValue = FlowGraphInteger.FromValue(intermediateValue.value);
	else if (className === "number" || className === "string" || className === "boolean") finalValue = intermediateValue.value[0];
	else if (intermediateValue && intermediateValue.value !== void 0) finalValue = intermediateValue.value;
	else if (Array.isArray(intermediateValue)) {
		if (intermediateValue.length > 0 && typeof intermediateValue[0] === "object" && intermediateValue[0] !== null && "eventData" in intermediateValue[0]) finalValue = intermediateValue.reduce((acc, val) => {
			if (!val.eventData) return acc;
			acc[val.id] = { type: getRichTypeByFlowGraphType(val.type) };
			if (typeof val.value !== "undefined") acc[val.id].value = defaultValueParseFunction("value", val, assetsContainer, scene);
			return acc;
		}, {});
		else finalValue = intermediateValue;
	} else finalValue = intermediateValue;
	return finalValue;
}
/**
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/flowGraphConnection.js
/**
...
var FlowGraphConnection = class {
	...
	*/
	connectTo(point) {
		if (this._connectionType === point._connectionType) throw new Error(`Cannot connect two points of type ${this.connectionType}`);
		if (this._isSingularConnection() && this._connectedPoint.length > 0 || point._isSingularConnection() && point._connectedPoint.length > 0) throw new Error("Max number of connections for point reached");
	...
	*/
	disconnectFrom(point, removeFromLocal = true) {
		...
		if (indexLocal === -1 || indexConnected === -1) return;
		if (removeFromLocal) this._connectedPoint.splice(indexLocal, 1);
	...
	*/
	disconnectFromAll() {
		for (const point of this._connectedPoint) this.disconnectFrom(point, false);
		this._connectedPoint.length = 0;
	}
	dispose() {
		for (const point of this._connectedPoint) this.disconnectFrom(point);
	}
	/**
	...
	*/
	serialize(serializationObject = {}) {
		...
		for (const point of this._connectedPoint) serializationObject.connectedPointIds.push(point.uniqueId);
	}
	/**
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/flowGraphDataConnection.pure.js
/** This file must only contain pure code and pure imports */
/**
...
var FlowGraphDataConnection = class extends FlowGraphConnection {
	...
	*/
	...
	set isDisabled(value) {
		...
		if (this._isDisabled) this.disconnectFromAll();
	}
	/**
	...
	*/
	setValue(value, context) {
		if (context._getConnectionValue(this) === value) return;
	...
	*/
	_reresolveDefaultValueForScene(scene) {
		...
		if (typeof value.getClassName === "function" && typeof value.getScene === "function") {
			if (value.getScene() === scene) return;
			reference = {
				id: value.id,
				name: value.name,
		...
		} else if (typeof value.className === "string" && (value.id || value.name) && value.value === void 0) reference = value;
		else return;
		const node = GetSceneNodeFromSerializedReference(reference, scene);
		if (node) this._defaultValue = node;
	}
	/**
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/FlowGraph/flowGraphBlock.js
/**
	...
	*/
	serialize(serializationObject = {}, _valueSerializeFunction = defaultValueSerializationFunction) {
		...
		if (this.config) {
			...
			for (const key of keys) _valueSerializeFunction(key, config[key], serializationObject.config);
		}
...
//#endregion
export { FlowGraphMatrix2D as A, RichTypeVector2 as C, getRichTypeByAnimationType as D, getAnimationTypeByFlowGraphType as E, FlowGraphInteger as M, RegisterFlowGraphInteger as N, getRichTypeByFlowGraphType as O, RichTypeString as S, RichTypeVector4 as T, RichTypeMatrix as _, FlowGraphConnectionType as a, RichTypeNumber as b, needsPathConverter as c, RichType as d, RichTypeAny as f, RichTypeFlowGraphInteger as g, RichTypeColor4 as h, FlowGraphConnection as i, FlowGraphMatrix3D as j, getRichTypeFromValue as k, FlowGraphTypes as l, RichTypeColor3 as m, FlowGraphDataConnection as n, defaultValueParseFunction as o, RichTypeBoolean as p, RegisterFlowGraphDataConnection as r, defaultValueSerializationFunction as s, FlowGraphBlock as t, RegisterFlowGraphRichTypes as u, RichTypeMatrix2D as v, RichTypeVector3 as w, RichTypeQuaternion as x, RichTypeMatrix3D as y };
