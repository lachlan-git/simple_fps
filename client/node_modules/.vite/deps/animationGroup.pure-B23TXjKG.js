...
import { n as Scene } from "./scene.pure-CZLmkwJ6.js";
import { n as Observable } from "./observable.pure-COgYOPLf.js";
import { i as TmpVectors, n as Quaternion, o as Vector3, t as Matrix, y as BuildArray } from "./math.vector.pure-BrntvQ1d.js";
import { t as EngineStore } from "./engineStore-BG5VoWzz.js";
import { n as _MissingSideEffect } from "./devTools-B-EiaEIm.js";
import { _ as _StaticOffsetValueSize, b as Node, c as AnimationMakeAnimationAdditive, g as _StaticOffsetValueQuaternion, h as _StaticOffsetValueColor4, l as AnimationParse, m as _StaticOffsetValueColor3, t as Animation, v as _StaticOffsetValueVector2, y as _StaticOffsetValueVector3 } from "./animation.pure-DqaKFoLR.js";
import { r as Tags } from "./math.size-Bg8qa66I.js";
import { t as PrecisionDate } from "./precisionDate-B6nrhdf_.js";
import { t as UniqueIdGenerator } from "./uniqueIdGenerator-FvVDzGPT.js";
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/runtimeAnimation.js
/**
...
var RuntimeAnimation = class {
	...
	*/
	constructor(target, animation, scene, host) {
		...
		*/
		...
		if (this._animation.dataType === Animation.ANIMATIONTYPE_MATRIX) this._animationState.workValue = Matrix.Zero();
		...
		if (events && events.length > 0) for (const e of events) this._events.push(e._clone());
	...
	_preparePath(target, targetIndex = 0) {
		...
		if (targetPropertyPath.length > 1) {
			...
			for (let index = 0; index < targetPropertyPath.length - 1; index++) {
				...
				if (property === void 0) throw new Error(`Invalid property (${name}) in property path (${targetPropertyPath.join(".")})`);
			}
		...
		if (this._activeTargets[targetIndex][this._targetPath] === void 0) throw new Error(`Invalid property (${this._targetPath}) in property path (${targetPropertyPath.join(".")})`);
	}
	/**
	...
	reset(restoreOriginal = false) {
		if (restoreOriginal) {
			if (this._target instanceof Array) {
				let index = 0;
				for (const target of this._target) {
					if (this._originalValue[index] !== void 0) this._setValue(target, this._activeTargets[index], this._originalValue[index], -1, index);
					index++;
				}
			} else if (this._originalValue[0] !== void 0) this._setValue(this._target, this._directTarget, this._originalValue[0], -1, 0);
		}
		this._offsetsCache = {};
		this._highLimitsCache = {};
		this._currentFrame = 0;
		this._blendingFactor = 0;
		for (let index = 0; index < this._events.length; index++) this._events[index].isDone = false;
	}
	/**
	...
	*/
	dispose() {
		...
		if (index > -1) this._animation.runtimeAnimations.splice(index, 1);
	}
	/**
	...
	*/
	...
	_getOriginalValues(targetIndex = 0) {
		...
		if (Animation.InheritOriginalValueFromActiveAnimations) {
			...
			for (let animIndex = 0; animIndex < activeAnimatables.length; animIndex++) {
				...
				for (let rtIndex = 0; rtIndex < runtimeAnimations.length; rtIndex++) {
					...
					if (rtAnim === this) continue;
					if (rtAnim._targetPath === this._targetPath) {
						for (let i = 0; i < rtAnim._activeTargets.length; i++) if (rtAnim._activeTargets[i] === target && rtAnim._originalValue[i] !== void 0) {
							if (rtAnim._originalValue[i] && rtAnim._originalValue[i].clone) this._originalValue[targetIndex] = rtAnim._originalValue[i].clone();
							else this._originalValue[targetIndex] = rtAnim._originalValue[i];
							return;
						}
					}
				}
		...
		if (target.getLocalMatrix && this._targetPath === "_matrix") originalValue = target.getLocalMatrix();
		else originalValue = target[this._targetPath];
		if (originalValue && originalValue.clone) this._originalValue[targetIndex] = originalValue.clone();
		else this._originalValue[targetIndex] = originalValue;
	}
	_registerTargetForLateAnimationBinding(runtimeAnimation, originalValue) {
		...
		if (!target._lateAnimationHolders) target._lateAnimationHolders = {};
		if (!target._lateAnimationHolders[runtimeAnimation.targetPath]) target._lateAnimationHolders[runtimeAnimation.targetPath] = {
			totalWeight: 0,
	...
	_setValue(target, destination, currentValue, weight, targetIndex) {
		...
		if (this._enableBlending && this._blendingFactor <= 1) {
			if (!this._originalBlendValue) {
				...
				if (originalValue.clone) this._originalBlendValue = originalValue.clone();
				else this._originalBlendValue = originalValue;
			}
			if (this._originalBlendValue.m) {
				if (Animation.AllowMatrixDecomposeForInterpolation) {
					if (this._currentValue) Matrix.DecomposeLerpToRef(this._originalBlendValue, currentValue, this._blendingFactor, this._currentValue);
					else this._currentValue = Matrix.DecomposeLerp(this._originalBlendValue, currentValue, this._blendingFactor);
				} else if (this._currentValue) Matrix.LerpToRef(this._originalBlendValue, currentValue, this._blendingFactor, this._currentValue);
				else this._currentValue = Matrix.Lerp(this._originalBlendValue, currentValue, this._blendingFactor);
			} else this._currentValue = Animation._UniversalLerp(this._originalBlendValue, currentValue, this._blendingFactor);
			const blendingSpeed = target && target.animationPropertiesOverride ? target.animationPropertiesOverride.blendingSpeed : this._animation.blendingSpeed;
			this._blendingFactor += blendingSpeed;
		} else if (!this._currentValue) {
			if (currentValue?.clone) this._currentValue = currentValue.clone();
		...
		else if (this._animationState.loopMode === Animation.ANIMATIONLOOPMODE_RELATIVE_FROM_CURRENT) {
			if (this._currentValue.addToRef) this._currentValue.addToRef(this._originalValue[targetIndex], destination[this._targetPath]);
			else destination[this._targetPath] = this._originalValue[targetIndex] + this._currentValue;
		} else destination[this._targetPath] = this._currentValue;
		if (target.markAsDirty) target.markAsDirty(this._animation.targetProperty);
	}
	/**
	...
	*/
	_getCorrectLoopMode() {
		if (this._target && this._target.animationPropertiesOverride) {
			if (this._target.getClassName?.() === "MorphTarget" && this._target._animationPropertiesOverride == null) return this._animation.loopMode;
	...
	*/
	goToFrame(frame, weight = -1) {
		...
		else if (frame > keys[keys.length - 1].frame) frame = keys[keys.length - 1].frame;
		const events = this._events;
		if (events.length) {
			for (let index = 0; index < events.length; index++) if (!events[index].onlyOnce) events[index].isDone = events[index].frame < frame;
		}
	...
	*/
	animate(elapsedTimeSinceAnimationStart, from, to, loop, speedRatio, weight = -1) {
		...
		if (!this._coreRuntimeAnimation) {
			if (from < this._minFrame || from > this._maxFrame) from = this._minFrame;
			if (to < this._minFrame || to > this._maxFrame) to = this._maxFrame;
			frameRange = to - from;
			let offsetValue;
			...
			if (yoyoMode) {
				...
				absoluteFrame = Math.abs(sin) * frameRange + from;
				const direction = sin >= 0 ? 1 : -1;
				if (this._yoyoDirection !== direction) yoyoLoop = true;
				...
				if (!this._offsetsCache[keyOffset]) {
					...
					switch (animation.dataType) {
						...
						case Animation.ANIMATIONTYPE_COLOR3:
							this._offsetsCache[keyOffset] = toValue.subtract(fromValue);
							break;
						case Animation.ANIMATIONTYPE_COLOR4: this._offsetsCache[keyOffset] = toValue.subtract(fromValue);
					}
			...
			if (offsetValue === void 0) switch (animation.dataType) {
				case Animation.ANIMATIONTYPE_FLOAT:
					offsetValue = 0;
					break;
				case Animation.ANIMATIONTYPE_QUATERNION:
				...
				case Animation.ANIMATIONTYPE_COLOR3:
					...
					break;
				case Animation.ANIMATIONTYPE_COLOR4: offsetValue = _StaticOffsetValueColor4;
			}
			if (this._host && this._host.syncRoot) {
				const syncRoot = this._host.syncRoot;
				const hostNormalizedFrame = (syncRoot.masterFrame - syncRoot.fromFrame) / (syncRoot.toFrame - syncRoot.fromFrame);
				currentFrame = from + frameRange * hostNormalizedFrame;
			} else if (absoluteFrame > 0 && from > to || absoluteFrame < 0 && from < to) currentFrame = returnValue && frameRange !== 0 ? to + absoluteFrame % frameRange : from;
			else currentFrame = returnValue && frameRange !== 0 ? from + absoluteFrame % frameRange : to;
			if (!yoyoMode && (speedRatio > 0 && this.currentFrame > currentFrame || speedRatio < 0 && this.currentFrame < currentFrame) || yoyoMode && yoyoLoop) {
				this._onLoop();
				for (let index = 0; index < events.length; index++) if (!events[index].onlyOnce) events[index].isDone = false;
		...
		this.setValue(currentValue, weight);
		if (events.length) {
			for (let index = 0; index < events.length; index++) if (frameRange >= 0 && currentFrame >= events[index].frame && events[index].frame >= from || frameRange < 0 && currentFrame <= events[index].frame && events[index].frame <= from) {
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/animatable.core.js
/**
* Class used to store an actual running animation
*/
var Animatable = class Animatable {
	/**
	...
	*/
	...
	set speedRatio(value) {
		for (let index = 0; index < this._runtimeAnimations.length; index++) this._runtimeAnimations[index]._prepareForSpeedRatioChange(value);
		this._speedRatio = value;
		if (this._goToFrame !== null) this.goToFrame(this._goToFrame);
	}
	/**
	...
	*/
	constructor(scene, target, fromFrame = 0, toFrame = 100, loopAnimation = false, speedRatio = 1, onAnimationEnd, animations, onAnimationLoop, isAdditive = false, playOrder = 0) {
		...
		*/
		...
		this._scene = scene;
		if (animations) this.appendAnimations(target, animations);
	...
	appendAnimations(target, animations) {
		for (let index = 0; index < animations.length; index++) {
			...
			newRuntimeAnimation._onLoop = () => {
				...
				if (this.onAnimationLoop) this.onAnimationLoop();
			};
	...
	*/
	getAnimationByTargetProperty(property) {
		...
		for (let index = 0; index < runtimeAnimations.length; index++) if (runtimeAnimations[index].animation.targetProperty === property) return runtimeAnimations[index].animation;
		return null;
	}
	/**
	...
	*/
	getRuntimeAnimationByTargetProperty(property) {
		const runtimeAnimations = this._runtimeAnimations;
		for (let index = 0; index < runtimeAnimations.length; index++) if (runtimeAnimations[index].animation.targetProperty === property) return runtimeAnimations[index];
		return null;
	}
	/**
	...
	*/
	reset() {
		const runtimeAnimations = this._runtimeAnimations;
		for (let index = 0; index < runtimeAnimations.length; index++) runtimeAnimations[index].reset(true);
		this._localDelayOffset = null;
		this._pausedDelay = null;
	}
	/**
	...
	*/
	disableBlending() {
		const runtimeAnimations = this._runtimeAnimations;
		for (let index = 0; index < runtimeAnimations.length; index++) runtimeAnimations[index].animation.enableBlending = false;
	}
	/**
	...
	*/
	goToFrame(frame, useWeight = false) {
		...
		for (let index = 0; index < runtimeAnimations.length; index++) runtimeAnimations[index].goToFrame(frame, useWeight ? this._weight : -1);
		...
		if (animationName || targetMask) {
			...
			if (idx > -1) {
				...
				for (let index = runtimeAnimations.length - 1; index >= 0; index--) {
					...
					if (animationName && runtimeAnimation.animation.name != animationName) continue;
					if (targetMask && !targetMask(runtimeAnimation.target)) continue;
					runtimeAnimation.dispose();
					runtimeAnimations.splice(index, 1);
				}
				if (runtimeAnimations.length == 0) {
					if (!useGlobalSplice) this._scene._activeAnimatables.splice(idx, 1);
					if (!skipOnAnimationEnd) this._raiseOnAnimationEnd();
				}
			}
		} else {
			const index = this._scene._activeAnimatables.indexOf(this);
			if (index > -1) {
				if (!useGlobalSplice) this._scene._activeAnimatables.splice(index, 1);
				const runtimeAnimations = this._runtimeAnimations;
				for (let index = 0; index < runtimeAnimations.length; index++) runtimeAnimations[index].dispose();
	...
	*/
	_animate(delay) {
		if (this._paused) {
			this.animationStarted = false;
			if (this._pausedDelay === null) this._pausedDelay = delay;
			return true;
		}
		...
		this._goToFrame = null;
		if (!Animatable.ProcessPausedAnimatables && this._weight === 0 && this._previousWeight === 0) return true;
		this._previousWeight = this._weight;
		let running = false;
		const runtimeAnimations = this._runtimeAnimations;
		let index = 0;
		for (; index < runtimeAnimations.length; index++) {
			const isRunning = runtimeAnimations[index].animate(delay - this._localDelayOffset, this.fromFrame, this.toFrame, this.loopAnimation, this._speedRatio, this._weight);
		...
		if (!running) {
			if (this.disposeOnEnd) {
				...
				this._scene._activeAnimatables.splice(index, 1);
				for (index = 0; index < runtimeAnimations.length; index++) runtimeAnimations[index].dispose();
			}
...
function ProcessLateAnimationBindingsForMatrices(holder) {
	if (holder.totalWeight === 0 && holder.totalAdditiveWeight === 0) return holder.originalValue;
	let normalizer = 1;
	...
	if (holder.totalWeight < 1) {
		...
		if (scale == 1) {
			if (holder.totalAdditiveWeight) skipOverride = true;
			else return originalAnimation.currentValue;
		}
	...
	if (!skipOverride) {
		...
		for (let animIndex = startIndex; animIndex < holder.animations.length; animIndex++) {
			...
			if (runtimeAnimation.weight === 0) continue;
...
function ProcessLateAnimationBindingsForQuaternions(holder, refQuaternion) {
	if (holder.totalWeight === 0 && holder.totalAdditiveWeight === 0) return refQuaternion;
	const originalAnimation = holder.animations[0];
	const originalValue = holder.originalValue;
	let cumulativeQuaternion = refQuaternion;
	if (holder.totalWeight === 0 && holder.totalAdditiveWeight > 0) cumulativeQuaternion.copyFrom(originalValue);
		...
		if (holder.totalAdditiveWeight === 0) return cumulativeQuaternion;
	} else if (holder.animations.length > 1) {
		let normalizer = 1;
		let quaternions;
		let weights;
		if (holder.totalWeight < 1) {
			...
			if (holder.animations.length === 2) {
				...
				if (holder.totalAdditiveWeight === 0) return refQuaternion;
			}
			quaternions = [];
			weights = [];
			normalizer = holder.totalWeight;
...
function ProcessLateAnimationBindings(scene) {
	if (!scene._registeredForLateAnimationBindings.length) return;
	for (let index = 0; index < scene._registeredForLateAnimationBindings.length; index++) {
		...
		for (const path in target._lateAnimationHolders) {
			...
			const originalAnimation = holder.animations[0];
			const originalValue = holder.originalValue;
			if (originalValue === void 0 || originalValue === null) continue;
			const matrixDecomposeMode = Animation.AllowMatrixDecomposeForInterpolation && originalValue.m;
			let finalValue = target[path];
			if (matrixDecomposeMode) finalValue = ProcessLateAnimationBindingsForMatrices(holder);
			else if (originalValue.w !== void 0) finalValue = ProcessLateAnimationBindingsForQuaternions(holder, finalValue || Quaternion.Identity());
			else {
				let startIndex = 0;
				let normalizer = 1;
				...
				if (holder.totalWeight < 1) {
					if (originalAnimationIsLoopRelativeFromCurrent) finalValue = originalValue.clone ? originalValue.clone() : originalValue;
					else if (originalAnimation && originalValue.scale) finalValue = originalValue.scale(1 - holder.totalWeight);
					else if (originalAnimation) finalValue = originalValue * (1 - holder.totalWeight);
					else if (originalValue.clone) finalValue = originalValue.clone();
					else finalValue = originalValue;
				} else if (originalAnimation) {
					normalizer = holder.totalWeight;
					const scale = originalAnimation.weight / normalizer;
					if (scale !== 1) {
						if (originalAnimation.currentValue.scale) finalValue = originalAnimation.currentValue.scale(scale);
						else finalValue = originalAnimation.currentValue * scale;
					} else finalValue = originalAnimation.currentValue;
					if (originalAnimationIsLoopRelativeFromCurrent) {
						if (finalValue.addToRef) finalValue.addToRef(originalValue, finalValue);
						else finalValue += originalValue;
					}
					startIndex = 1;
				}
				for (let animIndex = startIndex; animIndex < holder.animations.length; animIndex++) {
					...
					if (!scale) continue;
					else if (runtimeAnimation.currentValue.scaleAndAddToRef) runtimeAnimation.currentValue.scaleAndAddToRef(scale, finalValue);
					else finalValue += runtimeAnimation.currentValue * scale;
				}
				for (let animIndex = 0; animIndex < holder.additiveAnimations.length; animIndex++) {
					const runtimeAnimation = holder.additiveAnimations[animIndex];
...
function AddAnimationExtensions(sceneClass, boneClass) {
	if (boneClass) boneClass.prototype.copyAnimationRange = function(source, rangeName, frameOffset, rescaleAsRequired = false, skelDimensionsRatio = null) {
	...
	sceneClass.prototype._animate = function(customDeltaTime) {
		...
		for (let index = 0; index < animatables.length; index++) {
			...
			if (!animatable._animate(animationTime) && animatable.disposeOnEnd) index--;
		}
	...
	sceneClass.prototype.beginAnimation = function(target, from, to, loop, speedRatio = 1, onAnimationEnd, animatable, stopCurrent = true, targetMask, onAnimationLoop, isAdditive = false) {
		...
		if (from > to) speedRatio = -speedRatio;
		if (stopCurrent) this.stopAnimation(target, void 0, targetMask);
		if (!animatable) animatable = new Animatable(this, target, from, to, loop, speedRatio, onAnimationEnd, void 0, onAnimationLoop, isAdditive);
		const shouldRunTargetAnimations = targetMask ? targetMask(target) : true;
		if (target.animations && shouldRunTargetAnimations) animatable.appendAnimations(target, target.animations);
		if (target.getAnimatables) {
			const animatables = target.getAnimatables();
			for (let index = 0; index < animatables.length; index++) this.beginAnimation(animatables[index], from, to, loop, speedRatio, onAnimationEnd, animatable, stopCurrent, targetMask, onAnimationLoop);
		}
		animatable.reset();
		return animatable;
	};
	sceneClass.prototype.beginHierarchyAnimation = function(target, directDescendantsOnly, from, to, loop, speedRatio = 1, onAnimationEnd, animatable, stopCurrent = true, targetMask, onAnimationLoop, isAdditive = false) {
		...
		for (const child of children) result.push(this.beginAnimation(child, from, to, loop, speedRatio, onAnimationEnd, animatable, stopCurrent, targetMask, void 0, isAdditive));
		return result;
	};
	sceneClass.prototype.beginDirectAnimation = function(target, animations, from, to, loop, speedRatio = 1, onAnimationEnd, onAnimationLoop, isAdditive = false) {
		...
		if (from > to) speedRatio = -speedRatio;
		return new Animatable(this, target, from, to, loop, speedRatio, onAnimationEnd, animations, onAnimationLoop, isAdditive);
	};
	sceneClass.prototype.beginDirectHierarchyAnimation = function(target, directDescendantsOnly, animations, from, to, loop, speedRatio, onAnimationEnd, onAnimationLoop, isAdditive = false) {
		...
		for (const child of children) result.push(this.beginDirectAnimation(child, animations, from, to, loop, speedRatio, onAnimationEnd, onAnimationLoop, isAdditive));
		return result;
	};
	sceneClass.prototype.getAnimatableByTarget = function(target) {
		for (let index = 0; index < this._activeAnimatables.length; index++) if (this._activeAnimatables[index].target === target) return this._activeAnimatables[index];
		return null;
	};
	sceneClass.prototype.getAllAnimatablesByTarget = function(target) {
		const result = [];
		for (let index = 0; index < this._activeAnimatables.length; index++) if (this._activeAnimatables[index].target === target) result.push(this._activeAnimatables[index]);
		return result;
	};
	sceneClass.prototype.stopAnimation = function(target, animationName, targetMask) {
		const animatables = this.getAllAnimatablesByTarget(target);
		for (const animatable of animatables) animatable.stop(animationName, targetMask);
	};
	sceneClass.prototype.stopAllAnimations = function() {
		if (this._activeAnimatables) {
			for (let i = 0; i < this._activeAnimatables.length; i++) this._activeAnimatables[i].stop(void 0, void 0, true);
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Bones/bone.pure.js
...
*/
var Bone = class Bone extends Node {
	/** @internal */
	get _matrix() {
		this._compose();
	...
	/** @internal */
	set _matrix(value) {
		if (value.updateFlag === this._localMatrix.updateFlag && !this._needToCompose) return;
	...
	*/
	setParent(parent, updateAbsoluteBindMatrices = true) {
		...
		if (updateAbsoluteBindMatrices) this._updateAbsoluteBindMatrices();
		this.markAsDirty();
	}
	/**
	...
	*/
	...
	_decompose() {
		if (!this._needToDecompose) return;
	...
	*/
	updateMatrix(bindMatrix, updateAbsoluteBindMatrices = true, updateLocalMatrix = true) {
		...
		if (updateAbsoluteBindMatrices) this._updateAbsoluteBindMatrices();
		if (updateLocalMatrix) this._matrix = bindMatrix;
		else this.markAsDirty();
	}
	/**
	* @internal
	*/
	_updateAbsoluteBindMatrices(bindMatrix, updateChildren = true) {
		if (!bindMatrix) bindMatrix = this._bindMatrix;
		if (this.parent) bindMatrix.multiplyToRef(this.parent._absoluteBindMatrix, this._absoluteBindMatrix);
		else this._absoluteBindMatrix.copyFrom(bindMatrix);
		this._absoluteBindMatrix.invertToRef(this._absoluteInverseBindMatrix);
		if (updateChildren) for (let index = 0; index < this.children.length; index++) this.children[index]._updateAbsoluteBindMatrices();
	...
	*/
	...
	_updatePosition(vec, space = 0, tNode, translationMode = true) {
		...
		if (space == 0) {
			...
			} else lm.setTranslationFromFloats(vec.x, vec.y, vec.z);
		} else {
			const tmat = Bone._TmpMats[0];
			const tvec = Bone._TmpVecs[0];
			if (this.parent) {
				tmat.copyFrom(this.parent.getAbsoluteMatrix());
				if (tNode) tmat.multiplyToRef(tNode.getWorldMatrix(), tmat);
			} else Matrix.IdentityToRef(tmat);
			if (translationMode) tmat.setTranslationFromFloats(0, 0, 0);
			tmat.invert();
			...
			} else lm.setTranslationFromFloats(tvec.x, tvec.y, tvec.z);
		}
		this._markAsDirtyAndDecompose();
	}
	/**
	...
	*/
	scale(x, y, z, scaleChildren = false) {
		...
		this._markAsDirtyAndDecompose();
		if (scaleChildren) for (const child of this.children) child.scale(x, y, z, scaleChildren);
	}
	/**
	...
	*/
	setYawPitchRoll(yaw, pitch, roll, space = 0, tNode) {
		...
		if (!this._getAbsoluteInverseMatrixUnscaledToRef(rotMatInv, tNode)) return;
	...
	*/
	...
	_rotateWithMatrix(rmat, space = 0, tNode) {
		...
		if (parent && space == 1) {
			...
			} else parentScale.copyFrom(parent.getAbsoluteMatrix());
			parentScaleInv.copyFrom(parentScale);
			parentScaleInv.invert();
			lmat.multiplyToRef(parentScale, lmat);
			lmat.multiplyToRef(rmat, lmat);
			lmat.multiplyToRef(parentScaleInv, lmat);
		} else if (space == 1 && tNode) {
			parentScale.copyFrom(tNode.getWorldMatrix());
			parentScaleInv.copyFrom(parentScale);
			parentScaleInv.invert();
			lmat.multiplyToRef(parentScale, lmat);
			lmat.multiplyToRef(rmat, lmat);
			lmat.multiplyToRef(parentScaleInv, lmat);
		} else lmat.multiplyToRef(rmat, lmat);
	...
	_getAbsoluteInverseMatrixUnscaledToRef(rotMatInv, tNode) {
		...
		} else Matrix.IdentityToRef(scaleMatrix);
		rotMatInv.invert();
		if (isNaN(rotMatInv.m[0])) return false;
	...
	*/
	computeAbsoluteMatrices() {
		this._compose();
		if (this.parent) this._localMatrix.multiplyToRef(this.parent._absoluteMatrix, this._absoluteMatrix);
		else {
			this._absoluteMatrix.copyFrom(this._localMatrix);
			const poseMatrix = this._skeleton.getPoseMatrix();
			if (poseMatrix) this._absoluteMatrix.multiplyToRef(poseMatrix, this._absoluteMatrix);
		}
		const children = this.children;
		const len = children.length;
		for (let i = 0; i < len; i++) children[i].computeAbsoluteMatrices();
	}
	/**
	* Compute the absolute matrices of this bone and its children
	...
	*/
	getRotationQuaternionToRef(space = 0, tNode = null, result) {
		if (space == 0) {
			...
			if (tNode) amat.multiplyToRef(tNode.getWorldMatrix(), mat);
			else mat.copyFrom(amat);
	...
	*/
	getRotationMatrixToRef(space = 0, tNode, result) {
		if (space == 0) this.getLocalMatrix().getRotationMatrixToRef(result);
		else {
			const mat = Bone._TmpMats[0];
			const amat = this.getAbsoluteMatrix();
			if (tNode) amat.multiplyToRef(tNode.getWorldMatrix(), mat);
	...
	*/
	dispose() {
		...
		if (index !== -1) this._skeleton.bones.splice(index, 1);
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/animatable.pure.js
/** This file must only contain pure code and pure imports */
...
//#endregion
//#region ../../node_modules/.pnpm/@babylonjs+core@9.25.0/node_modules/@babylonjs/core/Animations/animationGroup.pure.js
/** This file must only contain pure code and pure imports */
var animationGroup_pure_exports = /* @__PURE__ */ __exportAll({
	AnimationGroup: () => AnimationGroup,
	AnimationGroupClipFrames: () => AnimationGroupClipFrames,
	AnimationGroupClipFramesInPlace: () => AnimationGroupClipFramesInPlace,
	AnimationGroupClipInPlace: () => AnimationGroupClipInPlace,
	AnimationGroupClipKeys: () => AnimationGroupClipKeys,
	AnimationGroupClipKeysInPlace: () => AnimationGroupClipKeysInPlace,
	AnimationGroupMakeAnimationAdditive: () => AnimationGroupMakeAnimationAdditive,
	AnimationGroupMergeAnimationGroups: () => AnimationGroupMergeAnimationGroups,
	AnimationGroupParse: () => AnimationGroupParse,
	RegisterAnimationGroup: () => RegisterAnimationGroup,
...
*/
var AnimationGroup = class AnimationGroup {
	/**
	...
	syncWithMask(forceUpdate = false) {
		...
		for (let i = 0; i < this._animatables.length; ++i) {
			...
			if (!this.mask || this.mask.disabled || this.mask.retainsTarget(animatable.target.name)) {
				...
				if (animatable.paused) animatable.restart();
			} else if (!animatable.paused) animatable.pause();
		}
	}
	/**
	...
	*/
	removeUnmaskedAnimations() {
		if (!this.mask || this.mask.disabled) return;
		for (let i = 0; i < this._animatables.length; ++i) {
			const animatable = this._animatables[i];
	...
	*/
	set speedRatio(value) {
		if (this._speedRatio === value) return;
		this._speedRatio = value;
		for (let index = 0; index < this._animatables.length; index++) {
			const animatable = this._animatables[index];
	...
	*/
	...
	set isAdditive(value) {
		if (this._isAdditive === value) return;
	...
	*/
	...
	set playOrder(value) {
		if (this._playOrder === value) return;
		this._playOrder = value;
		if (this._animatables.length > 0) {
			for (let i = 0; i < this._animatables.length; i++) this._animatables[i].playOrder = this._playOrder;
	...
	*/
	...
	set enableBlending(value) {
		if (this._enableBlending === value) return;
		this._enableBlending = value;
		if (value !== null) for (let i = 0; i < this._targetedAnimations.length; ++i) this._targetedAnimations[i].animation.enableBlending = value;
	}
	/**
	...
	*/
	...
	set blendingSpeed(value) {
		if (this._blendingSpeed === value) return;
		this._blendingSpeed = value;
		if (value !== null) for (let i = 0; i < this._targetedAnimations.length; ++i) this._targetedAnimations[i].animation.blendingSpeed = value;
	}
	/**
	...
	*/
	addTargetedAnimation(animation, target) {
		...
		if (this._from > keys[0].frame) this._from = keys[0].frame;
		if (this._to < keys[keys.length - 1].frame) this._to = keys[keys.length - 1].frame;
		if (this._enableBlending !== null) animation.enableBlending = this._enableBlending;
		if (this._blendingSpeed !== null) animation.blendingSpeed = this._blendingSpeed;
	...
	*/
	removeTargetedAnimation(animation) {
		for (let index = this._targetedAnimations.length - 1; index > -1; index--) if (this._targetedAnimations[index].animation === animation) this._targetedAnimations.splice(index, 1);
	}
	/**
	...
	*/
	normalize(beginFrame = null, endFrame = null) {
		...
		for (let index = 0; index < this._targetedAnimations.length; index++) {
			const keys = this._targetedAnimations[index].animation.getKeys();
	...
	_processLoop(animatable, targetedAnimation, index) {
		animatable.onAnimationLoop = () => {
			...
			if (this._animationLoopFlags[index]) return;
	...
	*/
	pause() {
		...
		this._isPaused = true;
		for (let index = 0; index < this._animatables.length; index++) this._animatables[index].pause();
	...
	*/
	play(loop) {
		if (this.isStarted && this._animatables.length && !this._shouldStart) {
			if (loop !== void 0) this.loopAnimation = loop;
			this.restart();
		} else {
			this.stop();
	...
	*/
	reset() {
		...
		for (let index = 0; index < this._animatables.length; index++) this._animatables[index].reset();
		return this;
	}
	/**
	...
	*/
	restart() {
		if (!this._isStarted) return this;
		for (let index = 0; index < this._animatables.length; index++) this._animatables[index].restart();
		this.syncWithMask();
		this._isPaused = false;
		this.onAnimationGroupPlayObservable.notifyObservers(this);
		return this;
	...
	*/
	stop(skipOnAnimationEnd = false) {
		...
		for (let index = 0; index < list.length; index++) list[index].stop(void 0, void 0, true, skipOnAnimationEnd);
		let curIndex = 0;
		for (let index = 0; index < this._scene._activeAnimatables.length; index++) {
			const animatable = this._scene._activeAnimatables[index];
			if (animatable._runtimeAnimations.length > 0) this._scene._activeAnimatables[curIndex++] = animatable;
			else if (skipOnAnimationEnd) this._checkAnimationGroupEnded(animatable, skipOnAnimationEnd);
		}
	...
	*/
	syncAllAnimationsWith(root) {
		for (let index = 0; index < this._animatables.length; index++) this._animatables[index].syncWith(root);
		return this;
	}
	/**
	...
	*/
	goToFrame(frame, useWeight = false) {
		if (!this._isStarted) return this;
		for (let index = 0; index < this._animatables.length; index++) this._animatables[index].goToFrame(frame, useWeight);
		return this;
	}
	/**
	...
	*/
	dispose() {
		...
		if (this._parentContainer) {
			...
			if (index > -1) this._parentContainer.animationGroups.splice(index, 1);
			this._parentContainer = null;
		}
		this.onAnimationEndObservable.clear();
	...
	_checkAnimationGroupEnded(animatable, skipOnAnimationEnd = false) {
		...
		if (idx > -1) this._animatables.splice(idx, 1);
		if (this._animatables.length === this._targetedAnimations.length - this._numActiveAnimatables) {
			this._isStarted = false;
			if (!skipOnAnimationEnd) this.onAnimationGroupEndObservable.notifyObservers(this);
			this._animatables.length = 0;
		}
	}
	/**
	...
	*/
	clone(newName, targetConverter, cloneAnimations = false, cloneAnimationKeys = false) {
		...
		for (const targetAnimation of this._targetedAnimations) newGroup.addTargetedAnimation(cloneAnimations ? targetAnimation.animation.clone(cloneAnimationKeys) : targetAnimation.animation, targetConverter ? targetConverter(targetAnimation.target) : targetAnimation.target);
		return newGroup;
	}
	/**
	...
	*/
	serialize() {
		...
		if (Tags && Tags.HasTags(this)) serializationObject.tags = Tags.GetTags(this);
		if (this.metadata) serializationObject.metadata = this.metadata;
		return serializationObject;
	}
	/**
...
function AnimationGroupMergeAnimationGroups(animationGroups, disposeSource = true, normalize = false, weight) {
	...
	if (normalize) for (const animationGroup of animationGroups) {
		if (animationGroup.from < beginFrame) beginFrame = animationGroup.from;
		if (animationGroup.to > endFrame) endFrame = animationGroup.to;
	}
	const mergedAnimationGroup = new AnimationGroup(animationGroups[0].name + "_merged", animationGroups[0].getScene(), weight);
	for (const animationGroup of animationGroups) {
		if (normalize) animationGroup.normalize(beginFrame, endFrame);
		for (const targetedAnimation of animationGroup.targetedAnimations) mergedAnimationGroup.addTargetedAnimation(targetedAnimation.animation, targetedAnimation.target);
		if (disposeSource) animationGroup.dispose();
	}
...
function AnimationGroupParse(parsedAnimationGroup, scene, targetLookup) {
	...
	for (let i = 0; i < parsedAnimationGroup.targetedAnimations.length; i++) {
		...
		if (target) animationGroup.addTargetedAnimation(animation, target);
	}
	if (Tags) Tags.AddTagsTo(animationGroup, parsedAnimationGroup.tags);
	if (parsedAnimationGroup.from !== null && parsedAnimationGroup.to !== null) animationGroup.normalize(parsedAnimationGroup.from, parsedAnimationGroup.to);
	if (parsedAnimationGroup.speedRatio !== void 0) animationGroupInternal._speedRatio = parsedAnimationGroup.speedRatio;
	if (parsedAnimationGroup.loopAnimation !== void 0) animationGroupInternal._loopAnimation = parsedAnimationGroup.loopAnimation;
	if (parsedAnimationGroup.isAdditive !== void 0) animationGroupInternal._isAdditive = parsedAnimationGroup.isAdditive;
	if (parsedAnimationGroup.weight !== void 0) animationGroupInternal._weight = parsedAnimationGroup.weight;
	if (parsedAnimationGroup.playOrder !== void 0) animationGroupInternal._playOrder = parsedAnimationGroup.playOrder;
	if (parsedAnimationGroup.enableBlending !== void 0) animationGroupInternal._enableBlending = parsedAnimationGroup.enableBlending;
	if (parsedAnimationGroup.blendingSpeed !== void 0) animationGroupInternal._blendingSpeed = parsedAnimationGroup.blendingSpeed;
	if (parsedAnimationGroup.metadata !== void 0) animationGroup.metadata = parsedAnimationGroup.metadata;
	return animationGroup;
}
/** @internal */
function AnimationGroupMakeAnimationAdditive(sourceAnimationGroup, referenceFrameOrOptions, range, cloneOriginal = false, clonedName) {
	let options;
	if (typeof referenceFrameOrOptions === "object") options = referenceFrameOrOptions;
	else options = {
	...
	if (options.cloneOriginalAnimationGroup) animationGroup = sourceAnimationGroup.clone(options.clonedAnimationGroupName || animationGroup.name);
	...
	if (options.clipKeys) {
		...
		const targetedAnimations = animationGroup.targetedAnimations;
		for (let index = 0; index < targetedAnimations.length; index++) {
			const keys = targetedAnimations[index].animation.getKeys();
			if (from > keys[0].frame) from = keys[0].frame;
			if (to < keys[keys.length - 1].frame) to = keys[keys.length - 1].frame;
		}
		const animationGroupInternal = animationGroup;
...
function AnimationGroupClipKeys(sourceAnimationGroup, fromKey, toKey, name, dontCloneAnimations) {
	return AnimationGroupClipKeysInPlace(sourceAnimationGroup.clone(name || sourceAnimationGroup.name), fromKey, toKey, dontCloneAnimations);
}
/**
...
function AnimationGroupClipFrames(sourceAnimationGroup, fromFrame, toFrame, name, dontCloneAnimations) {
	return AnimationGroupClipFramesInPlace(sourceAnimationGroup.clone(name || sourceAnimationGroup.name), fromFrame, toFrame, dontCloneAnimations);
}
/**
	...
	for (let index = 0; index < targetedAnimations.length; index++) {
		...
		for (let k = 0; k < keys.length; k++) {
			...
			if (!useFrame && k >= start && k <= end || useFrame && key.frame >= start && key.frame <= end) {
				...
				if (startFrame === Number.MAX_VALUE) startFrame = newKey.frame;
		...
		if (from > newKeys[0].frame) from = newKeys[0].frame;
		if (to < newKeys[newKeys.length - 1].frame) to = newKeys[newKeys.length - 1].frame;
...
//#endregion
export { RegisterTargetForLateAnimationBinding as _, AnimationGroupClipKeys as a, AnimationGroupMergeAnimationGroups as c, TargetedAnimation as d, animationGroup_pure_exports as f, Animatable as g, AddAnimationExtensions as h, AnimationGroupClipInPlace as i, AnimationGroupParse as l, Bone as m, AnimationGroupClipFrames as n, AnimationGroupClipKeysInPlace as o, RegisterAnimatable as p, AnimationGroupClipFramesInPlace as r, AnimationGroupMakeAnimationAdditive as s, AnimationGroup as t, RegisterAnimationGroup as u, RuntimeAnimation as v };
